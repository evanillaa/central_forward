...
Framework.Player.CreatePlayer = function(PlayerData)
	...
	self.Functions.AddMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			if amount > 50000 then
				TriggerEvent("qb-logs:server:SendLog", "playermoney", "AddMoney", "lightgreen", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** €"..amount .. " ("..moneytype..") added, new "..moneytype.." balans: "..self.PlayerData.money[moneytype], true)
			else
				TriggerEvent("qb-logs:server:SendLog", "playermoney", "AddMoney", "lightgreen", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** €"..amount .. " ("..moneytype..") added, new "..moneytype.." balans: "..self.PlayerData.money[moneytype])
			end
			TriggerClientEvent("qb-hud:client:money:change", self.PlayerData.source, moneytype, amount, false)
			return true
		end
		return false
	end
	...
	self.Functions.RemoveMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			if amount > 100000 then
				TriggerEvent("qb-logs:server:SendLog", "playermoney", "RemoveMoney", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** €"..amount .. " ("..moneytype..") eraf, new "..moneytype.." balans: "..self.PlayerData.money[moneytype], true)
			else
				TriggerEvent("qb-logs:server:SendLog", "playermoney", "RemoveMoney", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** €"..amount .. " ("..moneytype..") eraf, new "..moneytype.." balans: "..self.PlayerData.money[moneytype])
			end
			TriggerClientEvent("qb-hud:client:money:change", self.PlayerData.source, moneytype, amount, true)
			return true
		end
		return false
	end
	...
	self.Functions.SetMoney = function(moneytype, amount, reason)
		...
		if self.PlayerData.money[moneytype] ~= nil then
			...
			self.Functions.UpdatePlayerData()
			TriggerEvent("qb-logs:server:SendLog", "playermoney", "SetMoney", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** €"..amount .. " ("..moneytype..") gezet, new "..moneytype.." balans: "..self.PlayerData.money[moneytype])
			return true
		end
		return false
	end
	...
	self.Functions.AddItem = function(item, amount, slot, info)
		...
		if (totalWeight + (itemInfo["weight"] * amount)) <= Framework.Config.Player.MaxWeight then
			if (slot ~= nil and self.PlayerData.items[slot] ~= nil) and (self.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo["type"] == "item" and not itemInfo["unique"]) then
				...
				self.Functions.UpdatePlayerData()
				TriggerEvent("qb-logs:server:SendLog", "playerinventory", "AddItem", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** recieved item: [slot:" ..slot.."], itemname: " .. self.PlayerData.items[slot].name .. ", added amount: " .. amount ..", new total amount: ".. self.PlayerData.items[slot].amount)
				return true
			...
			elseif (itemInfo["unique"]) or (not slot or slot == nil) or (itemInfo["type"] == "weapon") then
				for i = 1, Config.Player.MaxInvSlots , 1 do
					if self.PlayerData.items[i] == nil then
						...
						self.Functions.UpdatePlayerData()
						TriggerEvent("qb-logs:server:SendLog", "playerinventory", "AddItem", "green", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** recieved item: [slot:" ..i.."], itemname: " .. self.PlayerData.items[i].name .. ", added amount: " .. amount ..", new total amount: ".. self.PlayerData.items[i].amount)
						return true
					end
				end
			end
	...
	self.Functions.RemoveItem = function(item, amount, slot)
		...
		if slot ~= nil then
			if self.PlayerData.items[slot].amount > amount then
				self.PlayerData.items[slot].amount = self.PlayerData.items[slot].amount - amount
				self.Functions.UpdatePlayerData()
				TriggerEvent("qb-logs:server:SendLog", "playerinventory", "RemoveItem", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** loses item: [slot:" ..slot.."], itemname: " .. self.PlayerData.items[slot].name .. ", removed amount: " .. amount ..", new total amount: ".. self.PlayerData.items[slot].amount)
				return true
			else
				self.PlayerData.items[slot] = nil
				self.Functions.UpdatePlayerData()
				TriggerEvent("qb-logs:server:SendLog", "playerinventory", "RemoveItem", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** loses item: [slot:" ..slot.."], itemname: " .. item .. ", removed amount: " .. amount ..", item removed")
				return true
			end
		else
	...
	self.Functions.SetInventory = function(items)
		...
		self.Functions.UpdatePlayerData()
		TriggerEvent("qb-logs:server:SendLog", "playerinventory", "SetInventory", "blue", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** items set: " .. json.encode(items))
	end

	self.Functions.ClearInventory = function()
		self.PlayerData.items = {}
		self.Functions.UpdatePlayerData()
		TriggerEvent("qb-logs:server:SendLog", "playerinventory", "ClearInventory", "red", "**"..GetPlayerName(self.PlayerData.source) .. " (citizenid: "..self.PlayerData.citizenid.." | id: "..self.PlayerData.source..")** inventory cleared")
	end
  ...
  Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_metadata` WHERE `citizenid` = '"..Framework.EscapeSqli(citizenid).."'", function(result)
	if result[1] ~= nil then
		if result[1].steam == GetPlayerIdentifiers(source)[1] then
	       ...
	       TriggerEvent("qb-logs:server:SendLog", "joinleave", "Character Deleted", "red", "**".. GetPlayerName(source) .. "** ("..GetPlayerIdentifiers(source)[1]..") deleted **"..citizenid.."**..")
		else
			TriggerClientEvent('qb-multicharacter:client:chooseChar', source)
			TriggerEvent("qb-logs:server:SendLog", "joinleave", "Character Cheats", "red", GetPlayerName(source) .." tries to remove char "..citizenid.." which is not his..", true)
		end
	else
		TriggerClientEvent('qb-multicharacter:client:chooseChar', source)
	    TriggerEvent("qb-logs:server:SendLog", "joinleave", "Character Cheats", "red", GetPlayerName(source) .." tries to remove char "..citizenid.." which is not his..", true)
