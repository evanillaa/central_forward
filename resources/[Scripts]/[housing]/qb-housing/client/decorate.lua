	...
	while true do
		...
		if DecoMode then
			...
			if GetDistanceBetweenCoords(camPos.x, camPos.y, camPos.z, Config.Houses[Currenthouse]['Coords']['Enter']['X'], Config.Houses[Currenthouse]['Coords']['Enter']['Y'], Config.Houses[Currenthouse]['Coords']['Enter']['Z'], false) > 50.0 then
				DisableEditMode()
				closeDecorateUI()
				Framework.Functions.Notify("You went out of range")
			end
		end
	end
end)
...
RegisterNetEvent("qb-housing:client:decorate")
AddEventHandler("qb-housing:client:decorate", function()
	Citizen.Wait(500)
	if IsInHouse then 
		if HasKey then 
			...
			Framework.Functions.Notify("You need the keys of this property!", "error")
		end
	else
		Framework.Functions.Notify("You are not inside a house", "error")
	end
end)
...
RegisterNetEvent("qb-housing:server:sethousedecorations")
AddEventHandler("qb-housing:server:sethousedecorations", function(house, decorations)
...
function LoadDecorations(house)
	if Config.Houses[house]['Decorations'] == nil or next(Config.Houses[house]['Decorations']) == nil then
		Framework.Functions.TriggerCallback('qb-housing:server:get:decorations', function(result)
...
function SaveDecorations()
	if Currenthouse ~= nil then
		...
		TriggerServerEvent("qb-housing:server:save:decorations", Currenthouse, ObjectList)
	end
end
