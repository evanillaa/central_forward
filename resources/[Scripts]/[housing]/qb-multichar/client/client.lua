...
Citizen.CreateThread(function()
	while true do
		...
		if NetworkIsSessionStarted() then
			TriggerEvent('qb-multichar:client:open:select')
			return
		end
	end
end)

RegisterNetEvent('qb-multichar:client:set:data')
AddEventHandler('qb-multichar:client:set:data', function(Slot, CitizenId, Model, Skin)
    Config.Peds[Slot]['CitizenId'] = CitizenId
    Config.Peds[Slot]['Model'] = tonumber(Model)
    Config.Peds[Slot]['Skin'] = json.decode(Skin)
end)

RegisterNetEvent('qb-multichar:client:open:select')
AddEventHandler('qb-multichar:client:open:select', function()
    DoScreenFadeOut(10)
    Citizen.Wait(1000)
    TriggerServerEvent('qb-multichar:server:set:data')
...
function CreatePeds()
    for k, v in pairs(Config.Peds) do
        ...
        exports['qb-assets']:RequestModelHash(LoadModel)
        ...
        if v['Skin'] ~= nil then
            TriggerEvent('qb-clothing:client:loadPlayerClothing', v['Skin'], CharPeds)
        end
...
RegisterNUICallback('GetCharacters', function(data, cb)
    Framework.Functions.TriggerCallback('qb-multichar:server:get:characters', function(Characters)
        cb(Characters)
    end)
end)
...
RegisterNUICallback('ClickedCard', function(data)
    exports['qb-assets']:RequestAnimationDict("friends@frj@ig_1")
...
RegisterNUICallback('SelectCharacter', function(data)
    ...
    Citizen.Wait(2500)
    exports['qb-assets']:RequestAnimationDict("anim@mp_fm_event@intro")
    ...
    TaskPlayAnim(FakeChars[data.Slot], 'anim@amb@nightclub@dancers@black_madonna_entourage@', 'hi_dance_facedj_09_v2_male^5', 3.0, 3.0, -1, 49, 0, false, false, false)
    Citizen.SetTimeout(2000, function()
        DoScreenFadeOut(150)
        Citizen.Wait(150)
        RemoveFakePeds()
        ResetCitizenId()
        TriggerServerEvent('qb-multichar:server:select:char', data.Citizenid)
    end)
end)
...
RegisterNUICallback('DeleteCharacter', function(data)
    ...
    Citizen.SetTimeout(1750, function()
        ...
        RemoveFakePeds()
        ResetCitizenId()
        TriggerServerEvent('qb-multichar:server:delete:char', data.Citizenid)
    end)
end)
...
RegisterNUICallback('CreateNewChar', function(data)
    ...
    Citizen.SetTimeout(2000, function()
        ...
        RemoveFakePeds()
        ResetCitizenId()
        TriggerServerEvent('qb-multichar:server:create:new:char', NewCharData)
    end)
end)
