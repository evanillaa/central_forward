...
$(document).on('click', '.multichar-card', function(e) {
 e.preventDefault();
  if (CanChoose) {
    $.post('http://qb-multichar/Click', JSON.stringify({}));
    if (SelectedChar !== null) {
      ...
      $.post('http://qb-multichar/ClickedCard', JSON.stringify({Slot: ClickedData.Slot}));
      if (ClickedData.CitizenId == "" || ClickedData.CitizenId == undefined) {
       $('.play-select-container').animate({bottom: "-20vh"}, 1000)
...
$(document).on('click', '.play-button', function(e) {
    ...
    if (CanChoose) {
        if (ClickedData.CitizenId != "" && ClickedData.Slot != null) {
          $.post('http://qb-multichar/Click', JSON.stringify({}));
          $.post('http://qb-multichar/SelectCharacter', JSON.stringify({Citizenid: ClickedData.CitizenId, Slot: ClickedData.Slot}));
          CloseCharScreen()
        }
    }
});
...
$(document).on('click', '.delete-button', function(e) {
    ...
    if (CanChoose) {
        if (ClickedData.CitizenId != "" && ClickedData.Slot != null) {
          $.post('http://qb-multichar/Click', JSON.stringify({}));
          $.post('http://qb-multichar/DeleteCharacter', JSON.stringify({Citizenid: ClickedData.CitizenId, Slot: ClickedData.Slot}));
          CloseCharScreen()
        }
    }
});
...
$(document).on('click', '.submit-button', function(e) {
    ...
    if (CanChoose) {
        ...
        if (CharName != "" && CharLastName != "" && CharBirthdate != "") {
          $.post('http://qb-multichar/Click', JSON.stringify({}));
          $.post('http://qb-multichar/CreateNewChar', JSON.stringify({FirstName: CharName, LastName: CharLastName, BirthDate: CharBirthdate, Gender: ResultGender, Slot: ClickedData.Slot}));
          CloseCharScreen()
        }
    }
});
...
SetupChars = function() {
  ...
  $.post('http://qb-multichar/GetCharacters', JSON.stringify({}), function(Characters){
     for (const [key, value] of Object.entries(Characters)) {
          $("#char-"+value.CharSlot).html('');
          $("#char-"+value.CharSlot).data('citizenid', value.Citizenid)
          $("#char-"+value.CharSlot).html('<p>'+ value.Name +'</p><p>Job: '+ value.Job +'</p><p>CID: '+ value.Citizenid +'</p><span class="tooltiptext">Select</span>');
      }
  });
}
...
ResetJs = function() {
 $("#char-1").html('<p>New Character</p><span class="tooltiptext">Create</span>');
 $("#char-2").html('<p>New Character</p><span class="tooltiptext">Create</span>');
 $("#char-3").html('<p>New Character</p><span class="tooltiptext">Create</span>');
...
CloseCharScreen = function() {
  ...
  $.post('http://qb-multichar/CloseNui', JSON.stringify({}));
}
