...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()
        ...
        Citizen.Wait(250)
        Framework.Functions.TriggerCallback("qb-bankrobbery:server:get:key:config", function(config)
            Config = config
        end)
           ...
           for k, v in pairs(Config.BankLocations[ClosestBank]['Lockers']) do
            ...
            if LockerDistance < 0.5 then
                if Config.BankLocations[ClosestBank]["Lockers"][k]['IsBusy'] then
                    ...
                    DrawText3D(Config.BankLocations[ClosestBank]["Lockers"][k]["X"], Config.BankLocations[ClosestBank]["Lockers"][k]["Y"], Config.BankLocations[ClosestBank]["Lockers"][k]["Z"], 'Not Available..')
                elseif Config.BankLocations[ClosestBank]["Lockers"][k]['IsOpend'] then
                    DrawText3D(Config.BankLocations[ClosestBank]["Lockers"][k]["X"], Config.BankLocations[ClosestBank]["Lockers"][k]["Y"], Config.BankLocations[ClosestBank]["Lockers"][k]["Z"], 'Opened..')
                    ...
                    DrawText3D(Config.BankLocations[ClosestBank]["Lockers"][k]["X"], Config.BankLocations[ClosestBank]["Lockers"][k]["Y"], Config.BankLocations[ClosestBank]["Lockers"][k]["Z"], '~g~E~s~ - Crack Safe')
...
RegisterNetEvent('qb-bankrobbery:client:set:open')
AddEventHandler('qb-bankrobbery:client:set:open', function(BankId, bool)
...
RegisterNetEvent('qb-bankrobbery:client:set:cards')
AddEventHandler('qb-bankrobbery:client:set:cards', function(BankId, CardData)
    Config.BankLocations[BankId]['card-type'] = CardData
end)

RegisterNetEvent('qb-bankrobbery:client:set:state')
AddEventHandler('qb-bankrobbery:client:set:state', function(BankId, LockerId, Type, bool)
  Config.BankLocations[BankId]['Lockers'][LockerId][Type] = bool
end)

RegisterNetEvent('qb-bankrobbery:client:use:card')
AddEventHandler('qb-bankrobbery:client:use:card', function(CardType)
    if ClosestBank ~= nil then
        ...
        if not Config.BankLocations[ClosestBank]['IsOpened'] then
            if Config.BankLocations[ClosestBank]['card-type'] == CardType then
                if CurrentCops >= Config.NeededCops then
                   Framework.Functions.TriggerCallback('qb-bankrobbery:server:HasItem', function(HasItem)
                       if HasItem then
                           Framework.Functions.TriggerCallback("qb-bankrobbery:server:get:status", function(Status)
                               if not Status then
                                   if Config.BankLocations[ClosestBank]['Alarm'] then
                                   TriggerServerEvent('qb-police:server:send:bank:alert', GetEntityCoords(PlayerPedId()), Framework.Functions.GetStreetLabel(), Config.BankLocations[ClosestBank]['CamId'])
                                   end
                                   TriggerEvent('qb-inventory:client:requiredItems', ItemsNeeded, false)
                                   ...
                                   TriggerEvent("utk_fingerprint:Start", 4, 6, 2, OnHackEnding)
                                    ...
                                    exports['qb-assets']:RequestAnimationDict("anim@heists@prison_heiststation@cop_reactions")
                                        ...
                                        if Success then
                                            TriggerEvent("utk_fingerprint:Start", 4, 6, 2, function(Outcome)
                                                if Outcome then
                                                    ...
                                                    TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items[Config.BankLocations[ClosestBank]['card-type']], "remove")
                                                    TriggerServerEvent('qb-bankrobbery:server:set:open', ClosestBank, true)
                                                else
                                                    StopAnimTask(PlayerPedId(), "anim@heists@prison_heiststation@cop_reactions", "cop_b_idle", 3.0, 3.0, -1, 2, 0, 0, 0, 0)
                                   ...
                                   Framework.Functions.Notify("Another robbery has already started..", "error")
                               end
                           end)
                       else
                    ...
                    Framework.Functions.Notify("Not enough cops! ("..Config.NeededCops.." Needed)", "info")
                end
            else
                Framework.Functions.Notify("Not the correct card...", "error")
            end
        end
      end
    end
...
function LockpickLocker(LockerId)
    ...
    if Type == 'lockpick' then
        Framework.Functions.TriggerCallback("qb-bankrobbery:server:HasLockpickItems", function(HasItem)
            if HasItem then
            TriggerServerEvent('qb-bankrobbery:server:set:state', ClosestBank, LockerId, 'IsBusy', true)
              exports['qb-lockpick']:OpenLockpickGame(function(Success)
                  if Success then
                      Framework.Functions.Progressbar("break-safe", "Breaking Open..", math.random(10000, 15000), false, true, {
		                  ...
		                  flags = 8,
                           ...
                           TriggerServerEvent('qb-bankrobbery:server:random:reward', math.random(1,3), ClosestBank)
                           TriggerServerEvent('qb-bankrobbery:server:set:state', ClosestBank, LockerId, 'IsBusy', false) 
                           TriggerServerEvent('qb-bankrobbery:server:set:state', ClosestBank, LockerId, 'IsOpend', true) 
                ...
                Framework.Functions.Notify("You dont have any lockpicks..", "error")
            end
        end)
        ...
        if HasItem then           
            Framework.Functions.TriggerCallback("Framework:HasItem", function(SHasItem)
                if SHasItem then
                    ...
                    TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items['drill-bit'], "remove")
                    TriggerServerEvent('qb-bankrobbery:server:set:state', ClosestBank, LockerId, 'IsBusy', true)
                    PlaySoundFrontend(-1, "BASE_JUMP_PASSED", "HUD_AWARDS", 1);
                    exports['qb-assets']:RequestAnimationDict("anim@heists@fleeca_bank@drilling")
                    TaskPlayAnim(PlayerPedId(), 'anim@heists@fleeca_bank@drilling', 'drill_straight_idle' , 3.0, 3.0, -1, 1, 0, false, false, false)
                    exports['qb-assets']:AddProp('Drill')
                    exports['minigame-drill']:StartDrilling(function(Success)
                       if Success then
                           ...
                           TriggerServerEvent('qb-bankrobbery:server:random:reward', math.random(1,3))
                           TriggerServerEvent('qb-bankrobbery:server:set:state', ClosestBank, LockerId, 'IsBusy', false) 
                           TriggerServerEvent('qb-bankrobbery:server:set:state', ClosestBank, LockerId, 'IsOpend', true) 
                       else
                           exports['qb-assets']:RemoveProp()
                    ...
                    Framework.Functions.Notify('You are missing something..', 'error', 2500)
                end
            end, 'drill-bit')
        else
            Framework.Functions.Notify("You have no drill..", "error")
        end
      end, 'drill')
    end
end
