            ...
            for k, v in pairs(Config.SpecialBanks) do
                    ...
                    for Troll, Trolly in pairs(Config.Trollys) do
                        ...
                        if Distance < 1.5 then
                            ...
                            if not Trolly['Open-State'] then
                             ...
                             DrawText3D(Trolly['Grab-Coords']['X'], Trolly['Grab-Coords']['Y'], Trolly['Grab-Coords']['Z'],'~g~E~s~ - Grab Money')
                   ...
                   if not v['Open'] then
                        ...
                        if Area < 1.3 then
                            ...
                            TriggerEvent('qb-inventory:client:requiredItems', PacificItems, true)
                            end
                        end
                   end
                end
    ...
    while true do
        ...
        if LoggedIn then
            if GrabbingMoney then
              TriggerServerEvent('qb-bankrobbery:server:rob:pacific:money')
              Citizen.Wait(1000)
            end
        end
    end
...
RegisterNetEvent('qb-bankrobbery:client:use:black-card')
AddEventHandler('qb-bankrobbery:client:use:black-card', function()
    for k, v in pairs(Config.SpecialBanks) do 
            if not v['Open'] then
                local Area = GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), v['Hack-Door']['X'], v['Hack-Door']['Y'], v['Hack-Door']['Z'], true)
                if Area < 1.35 then
                    if CurrentCops >= Config.NeededCopsPacific then
                        Framework.Functions.TriggerCallback('qb-bankrobbery:server:HasItem', function(HasItem)
                            if HasItem then
                                Framework.Functions.TriggerCallback("qb-bankrobbery:server:get:status", function(Status)
                                    if not Status then
                                        TriggerServerEvent('qb-police:server:send:big:bank:alert', GetEntityCoords(PlayerPedId()), Framework.Functions.GetStreetLabel())
                                        TriggerEvent('qb-inventory:client:set:busy', true)
                                        TriggerEvent('qb-inventory:client:requiredItems', PacificItems, false)
                                        ...
                                        exports['minigame-phone']:StartHack(math.random(1,4), 130, function(Success)
                                            if Success then
                                                TriggerEvent("utk_fingerprint:Start", 2, 2, 2, function(Outcome)
                                                    if Outcome then
                                                        CreateTrollys()
                                                        TriggerServerEvent('qb-bankrobbery:server:pacific:start')
                                                        TriggerServerEvent('Framework:Server:RemoveItem', 'black-card', 1)
                                                        TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items['black-card'], "remove")
                                        ...
                                        Framework.Functions.Notify("Another robbery already started..", "error")
                                    end
                                end)
                            else
                        ...
                        Framework.Functions.Notify("Not enough cops! ("..Config.NeededCopsPacific.." Needed)", "info")
                    end
                end
            end
    end
...
RegisterNetEvent('qb-bankrobbery:client:pacific:start')
AddEventHandler('qb-bankrobbery:client:pacific:start', function()
    OpenPacificDoor()
    Config.SpecialBanks[1]['Open'] = true
end)

RegisterNetEvent('qb-bankrobbery:client:clear:trollys')
AddEventHandler('qb-bankrobbery:client:clear:trollys', function()
    Count = 0
...
RegisterNetEvent('qb-bankrobbery:client:set:trolly:state')
AddEventHandler('qb-bankrobbery:client:set:trolly:state', function(TrollyNumber, bool)
...
function GetMoneyFromTrolly(TrollyNumber)
    ...
    GrabbingMoney = false
    TriggerServerEvent('qb-bankrobbery:server:set:trolly:state', TrollyNumber, true)
