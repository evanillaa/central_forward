...
Framework.Functions.CreateCallback('qb-banktruck:server:GetConfig', function(source, cb)
    cb(Config)
end)
...
local Player = Framework.Functions.GetPlayer(source)
 ...
 TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items['green-card'], "remove")
end)
...
AddEventHandler('blijf:uit:mijn:scripts:rewards', function()
    ...
    elseif RandomWaarde >= 31 and RandomWaarde <= 50 then
        ...
        TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['gold-rolex'], "add")
    elseif RandomWaarde >= 51 and RandomWaarde <= 80 then 
        local AmountGoldStuff = math.random(6,25)
        Player.Functions.AddItem('gold-necklace', AmountGoldStuff)
        TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['gold-necklace'], "add")
    ...
    elseif RandomWaarde == 91 or RandomWaarde == 98 or RandomWaarde == 85 or RandomWaarde == 65 then
        ...
        TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['gold-bar'], "add") 
        TriggerEvent("qb-log:server:CreateLog", "banktruck", "Banktruck Rewards", "green", "**Player:** "..GetPlayerName(src).." (citizenid: *"..Player.PlayerData.citizenid.."*)\n**Gekregen: **Goud Staaf\n**Aantal: **"..RandomAmount)
    elseif RandomWaarde == 26 or RandomWaarde == 52 then 
        Player.Functions.AddItem('yellow-card', 1)
        TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['yellow-card'], "add") 
        TriggerEvent("qb-log:server:CreateLog", "banktruck", "Banktruck Rewards", "green", "**Player:** "..GetPlayerName(src).." (citizenid: *"..Player.PlayerData.citizenid.."*)\n**Gekregen: **Gele Kaart\n**Aantal:** 1x")
    end
end)
