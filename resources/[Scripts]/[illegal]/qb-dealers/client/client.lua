...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
 Citizen.SetTimeout(450, function()
  ...
  Citizen.Wait(250)
  Framework.Functions.TriggerCallback("qb-dealers:server:get:config", function(config)
...
RegisterNetEvent('qb-dealers:client:open:dealer')
AddEventHandler('qb-dealers:client:open:dealer', function()
    Citizen.SetTimeout(350, function()
        ...
        if CurrentDealer ~= nil then 
          ...
          TriggerServerEvent("qb-inventory:server:OpenInventory", "shop", "Dealer_"..CurrentDealer, Shop)
        end
    end)
end)

RegisterNetEvent('qb-dealers:client:update:dealer:items')
AddEventHandler('qb-dealers:client:update:dealer:items', function(ItemData, Amount)
    TriggerServerEvent('qb-dealers:server:update:dealer:items', ItemData, Amount, CurrentDealer)
end)

RegisterNetEvent('qb-dealers:client:set:dealer:items')
AddEventHandler('qb-dealers:client:set:dealer:items', function(ItemData, Amount, Dealer)
    Config.Dealers[Dealer]["Products"][ItemData.slot].amount = Config.Dealers[Dealer]["Products"][ItemData.slot].amount - Amount
end)

RegisterNetEvent('qb-dealers:client:reset:items')
AddEventHandler('qb-dealers:client:reset:items', function()
