...
Citizen.CreateThread(function()
    ...
    RegisterKeyMapping('+gangsPress', 'Gangs General Use', 'keyboard', 'E')
...
AddEventHandler("gangs:openStash", function(gang)
    TriggerServerEvent("qb-inventory:server:OpenInventory", "stash", gang, {
        maxweight = 4000000,
        slots = 500,
    })
    TriggerEvent("qb-inventory:client:SetCurrentStash", gang)
end)
...
AddEventHandler("gangs:vehiclesMenu", function(gang, money)
    ...
    if IsPedInAnyVehicle(PlayerPedId()) then
        items = {{ model = "store", label = "Store vehicle", icon = "fas fa-history" }}
    end
