...
Framework.Functions.CreateCallback("qb-houseplants:server:get:config", function(source, cb)
    cb(Config)
end)
...
Citizen.CreateThread(function()
    Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_house_plants`", function(result)
        for k, v in pairs(result) do
            ...
            TriggerClientEvent('qb-houseplants:client:sync:plant:data', -1, Config.Plants)
        end
    end)
end)

RegisterServerEvent('qb-houseplants:server:destroy:plant')
AddEventHandler('qb-houseplants:server:destroy:plant', function(HouseId, RemoveId, IsDead)
    local src = source
    ...
    Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_house_plants` WHERE `houseid` = '"..HouseId.."'", function(result)
        ...
        if IsDead then
            ...
            TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['wet-tak'], "add")
        end
    end)
end)

RegisterServerEvent('qb-houseplants:server:add:plant')
AddEventHandler('qb-houseplants:server:add:plant', function(HouseId, NewPlant)
    Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_house_plants` WHERE `houseid` = '"..HouseId.."'", function(result)
...
RegisterServerEvent('qb-houseplants:server:feed:plant')
AddEventHandler('qb-houseplants:server:feed:plant', function(HouseId, PlantId)
    local src = source
    local Player = Framework.Functions.GetPlayer(src)
    if Config.Plants[HouseId] ~= nil then
       if Config.Plants[HouseId][PlantId]['Food'] < 100 then
            ...
            TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['weed-nutrition'], "remove")
            Player.Functions.RemoveItem('weed-nutrition', 1)
       end
    end
end)

RegisterServerEvent('qb-houseplants:server:harvest:plant')
AddEventHandler('qb-houseplants:server:harvest:plant', function(HouseId, PlantId, Amount)
   local src = source
   local Player = Framework.Functions.GetPlayer(src)
   ...
   if Config.Plants[HouseId] ~= nil then
        if PlasticBag.amount >= RandomWeedAmount then
            ...
            if Config.Plants[HouseId][PlantId]['Sort'] == 'White-Widow' then
                ...
                TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['weed_white-widow'], "add")
            ...
            elseif Config.Plants[HouseId][PlantId]['Sort'] == 'Skunk' then
                ...
                TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['weed_skunk'], "add")
                if math.random(1,5) <= 2 and Config.Plants[HouseId][PlantId]['Gender'] == 'Man' then
            ...
            elseif Config.Plants[HouseId][PlantId]['Sort'] == 'Purple-Haze' then
                ...
                TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['weed_purple-haze'], "add")
                if math.random(1,5) <= 2 and Config.Plants[HouseId][PlantId]['Gender'] == 'Man' then
            ...
            elseif Config.Plants[HouseId][PlantId]['Sort'] == 'Og-Kush' then
                ...
                TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['weed_og-kush'], "add")
                if math.random(1,5) <= 2 and Config.Plants[HouseId][PlantId]['Gender'] == 'Man' then
            ...
            elseif Config.Plants[HouseId][PlantId]['Sort'] == 'Amnesia' then
                ...
                TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['weed_amnesia'], "add")
                if math.random(1,5) <= 2 and Config.Plants[HouseId][PlantId]['Gender'] == 'Man' then
            ...
            elseif Config.Plants[HouseId][PlantId]['Sort'] == 'AK47' then
                ...
                TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['weed_ak47'], "add")
                if math.random(1,5) <= 2 and Config.Plants[HouseId][PlantId]['Gender'] == 'Man' then
                    Player.Functions.AddItem('ak47-seed', Amount)
                end
            end
            TriggerEvent('qb-houseplants:server:destroy:plant', HouseId, PlantId, false)
        else
            TriggerClientEvent('Framework:Notify', src, 'You dont have enough bags..', 'error', 3500)
        end
    end
end)
...
function RefreshPlants(HouseId)
    Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_house_plants` WHERE `houseid` = '"..HouseId.."'", function(result)
        ...
        TriggerClientEvent('qb-houseplants:client:sync:plant:data', -1, Config.Plants)
        TriggerClientEvent('qb-houseplants:client:sync:plants', -1, HouseId)
    end)
end