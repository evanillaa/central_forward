                    ...
                    if OffSets ~= nil then
                        ...
                        for k, v in pairs(Config.HouseLocations[CurrentHouse]['Lockers']) do
                            if (GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, v['Coords']['X'], v['Coords']['Y'], v['Coords']['Z'], true) < 1.5) then
                                ...
                                if Config.HouseLocations[CurrentHouse]['Lockers'][k]['Busy'] then Text = '~o~Robbing..' elseif Config.HouseLocations[CurrentHouse]['Lockers'][k]['Opened'] then Text = '~r~Empty..' end
  ...
  Framework.Functions.TriggerCallback('Framework:HasItem', function(HasItem)
    if CurrentHouse ~= nil then
        if CurrentCops >= Config.CopsNeeded then
               ...
               exports['qb-lockpick']:OpenLockpickGame(function(Success)
                if Success then
                    ...
                    if math.random(1,100) < 19 then
                        ...
                        Framework.Functions.Notify("You dropped an piece of evidence (blood)", "error")
                    end
                end
               end)
            else
            ...
            Framework.Functions.Notify("Nog enough cops! ("..Config.CopsNeeded.." Needed)", "info")
        end
    end
  end, "toolkit")
end)
...
function LockpickFinish(Success)
 if Success then
   ...
   LockpickAnim(Time)
   Framework.Functions.Progressbar("lockpick-door", "Breaking In..", Time, false, true, {
...
function OpenLocker(LockerId)
  ...
  Framework.Functions.Progressbar("lockpick-locker", "Searching..", Time, false, true, {
    ...
    disableCombat = true,
      ...
      TriggerServerEvent('qb-houserobbery:server:locker:reward', math.random(1,3))
