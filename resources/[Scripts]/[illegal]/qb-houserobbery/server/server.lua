...
Framework.Functions.CreateCallback('qb-houserobbery:server:get:config', function(source, cb)
  cb(Config)
end)

-- Code

RegisterServerEvent('qb-houserobbery:server:set:door:status')
AddEventHandler('qb-houserobbery:server:set:door:status', function(RobHouseId, bool)
 Config.HouseLocations[RobHouseId]['Opened'] = bool
 TriggerClientEvent('qb-houserobbery:client:set:door:status', -1, RobHouseId, bool)
 ResetHouse(RobHouseId)
end)

RegisterServerEvent('qb-houserobbery:server:set:locker:state')
AddEventHandler('qb-houserobbery:server:set:locker:state', function(RobHouseId, LockerId, Type, bool)
 Config.HouseLocations[RobHouseId]['Lockers'][LockerId][Type] = bool
 TriggerClientEvent('qb-houserobbery:client:set:locker:state', -1, RobHouseId, LockerId, Type, bool)
end)

RegisterServerEvent('qb-houserobbery:server:locker:reward')
AddEventHandler('qb-houserobbery:server:locker:reward', function()
  ...
  elseif RandomValue == 32 or RandomValue == 34 or RandomValue == 40 then
    ...
    if SubValue == 1 then
      ...
      TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items['rifle-trigger'], "add")
    else
      Player.Functions.AddItem('rifle-body', 1)
      TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items['rifle-body'], "add")
    end
  else
...
RegisterServerEvent('qb-houserobbery:server:recieve:extra')
AddEventHandler('qb-houserobbery:server:recieve:extra', function(CurrentHouse, Id)
  local Player = Framework.Functions.GetPlayer(source)
  Player.Functions.AddItem(Config.HouseLocations[CurrentHouse]['Extras'][Id]['Item'], 1)
  TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items[Config.HouseLocations[CurrentHouse]['Extras'][Id]['Item']], "add")
  Config.HouseLocations[CurrentHouse]['Extras'][Id]['Stolen'] = true
  TriggerClientEvent('qb-houserobbery:client:set:extra:state', -1, CurrentHouse, Id, true)
end)
...
function ResetHouse(HouseId)
  Citizen.SetTimeout((1000 * 60) * 15, function()
      ...
      TriggerClientEvent('qb-houserobbery:server:reset:state', -1, HouseId)
  end)
end