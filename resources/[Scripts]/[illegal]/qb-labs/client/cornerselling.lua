...
RegisterNetEvent('qb-illegal:client:toggle:corner:selling')
AddEventHandler('qb-illegal:client:toggle:corner:selling', function()
    ...
    if Distance < 1250 then
        Framework.Functions.TriggerCallback('qb-illegal:server:has:drugs', function(HasDrugs)
            if HasDrugs then
...
RegisterNetEvent('qb-illegal:client:sell:to:ped')
AddEventHandler('qb-illegal:client:sell:to:ped', function()
    if CurrentNpc ~= nil and Config.IsCornerSelling then
        Framework.Functions.TriggerCallback('qb-illegal:server:get:drugs:items', function(DrugsResult)
            ...
            if SuccesChance <= 30 then
                ...
                HasOffer = false
                HasNpcFound = false
                TriggerServerEvent('qb-police:server:alert:cornerselling', GetEntityCoords(CurrentNpc), Framework.Functions.GetStreetLabel())
                SetEveryoneIgnorePlayer(PlayerId(), false)
                Citizen.Wait(150)
                TaskSmartFleePed(CurrentNpc, PlayerPedId(), 100.0, 15000)
                table.insert(LastPed, CurrentNpc) 
...
function SellNowToPed(Price, Item, Amount)
    ...
    Framework.Functions.Progressbar("open-brick", "Making deal..", 7500, false, true, {
        ...
        TriggerServerEvent('qb-illegal:server:finish:corner:selling', Price, Item, Amount)
