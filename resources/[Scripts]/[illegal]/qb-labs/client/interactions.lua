...
function SpawnNpcs()
    for k, v in pairs(Config.PedInteraction) do
        exports['qb-assets']:RequestModelHash(v['Model'])
...
RegisterNetEvent('qb-illegal:client:talk:to:npc')
AddEventHandler('qb-illegal:client:talk:to:npc', function()
    Citizen.SetTimeout(150, function()
        ...
        if Config.PedInteraction[CurrentNpc]['InteractAnimation'] ~= nil then
            exports['qb-assets']:RequestAnimationDict(Config.PedInteraction[CurrentNpc]['InteractAnimation']['AnimDict'])
        ...
        if Config.PedInteraction[CurrentNpc]['Name'] == 'Tyrone' or Config.PedInteraction[CurrentNpc]['Name'] == 'Tyries' or Config.PedInteraction[CurrentNpc]['Name'] == 'Dodge' or Config.PedInteraction[CurrentNpc]['Name'] == 'Dwayne' or Config.PedInteraction[CurrentNpc]['Name'] == 'Meth' or Config.PedInteraction[CurrentNpc]['Name'] == 'Coke' or Config.PedInteraction[CurrentNpc]['Name'] == 'Achmed' or Config.PedInteraction[CurrentNpc]['Name'] == 'Richard' then
            ...
            TriggerServerEvent("qb-inventory:server:OpenInventory", "shop", "StreetDealer_"..Config.PedInteraction[CurrentNpc]['Name'], Shop)
        elseif Config.PedInteraction[CurrentNpc]['Name'] == 'Locksmith' or Config.PedInteraction[CurrentNpc]['Name'] == 'Toolshop1' or Config.PedInteraction[CurrentNpc]['Name'] == 'Toolshop2' or Config.PedInteraction[CurrentNpc]['Name'] == 'Fisher' then
            TriggerEvent('qb-stores:server:open:shop')
        elseif Config.PedInteraction[CurrentNpc]['Name'] == 'Bank' then
            Framework.Functions.TriggerCallback('Framework:HasItem', function(HasItem)
                if HasItem then
                    TriggerServerEvent('qb-burgershot:server:sell:tickets')
                else
                  Framework.Functions.Notify("You have no receipts..", "error")
                end
            end, 'burger-ticket')
        elseif Config.PedInteraction[CurrentNpc]['Name'] == 'SellElectronics' then
            TriggerServerEvent('qb-illegal:server:sell:electrnoics')
        end
    end)
end)