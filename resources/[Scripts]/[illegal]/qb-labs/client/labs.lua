...
RegisterNetEvent('qb-illegal:client:use:key')
AddEventHandler('qb-illegal:client:use:key', function(KeyName)
    if not InsideLab then
        if CurrentLab ~= nil then
            if KeyName == Config.Labs[CurrentLab]['KeyName'] then
                EnterCurrentLab()
            else
                Framework.Functions.Notify('This is not the right key..', 'error')
            end
        end
    end
end)

RegisterNetEvent('qb-illegal:client:sync:inventory')
AddEventHandler('qb-illegal:client:sync:inventory', function(LabId, data)
