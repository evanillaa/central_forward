            ...
            if InsideLab and Config.Labs[CurrentLab]['Name'] == 'Methlab' then
                ...
                if Distance < 1.2 then
                    ...
                    if IsControlJustReleased(0, 38) then
		    			...
		    			TriggerServerEvent("qb-inventory:server:OpenInventory", "methcrafting", math.random(1, 99), MethCrafting)
                    end
                end
...
RegisterNetEvent('qb-illegal:client:start:cooking')
AddEventHandler('qb-illegal:client:start:cooking', function()
    Cooking = true
    while Cooking do
        Citizen.Wait(0)
        if Config.CookTimer > 0 then
        ...
        if Config.CookTimer == 0 then
            ...
            if Random <= Config.ExplosionChance then
                if CurrentLab ~= nil then
                    TriggerServerEvent('qb-illegal:server:reset:meth', CurrentLab)
                    ExplosionMethFail()
                end
            end
            Cooking = false
...
RegisterNetEvent('qb-illegal:client:sync:meth')
AddEventHandler('qb-illegal:client:sync:meth', function(ConfigData, LabId, Reset)
    ...
    if not Config.Labs[CurrentLab]['Ingredients']['meth-ingredient-1'] then
        Framework.Functions.TriggerCallback('Framework:HasItem', function(HasItem)
            if HasItem then
                ...
                TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items['meth-ingredient-1'], "remove")
                TriggerServerEvent('qb-illegal:server:add:ingredient', CurrentLab, 'meth-ingredient-1', true, 1)
            else
    ...
    elseif not Config.Labs[CurrentLab]['Ingredients']['meth-ingredient-2'] then
        Framework.Functions.TriggerCallback('Framework:HasItem', function(HasItem)
            if HasItem then
                ...
                TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items['meth-ingredient-2'], "remove")
                TriggerServerEvent('qb-illegal:server:add:ingredient', CurrentLab, 'meth-ingredient-2', true, 1)
            else
                Framework.Functions.Notify('Missing something..', 'error')
            end
...
function GetMeth()
    TriggerServerEvent('qb-illegal:server:get:meth', math.random(25, 30), CurrentLab)
end