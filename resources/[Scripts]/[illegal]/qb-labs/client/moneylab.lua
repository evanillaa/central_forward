...
function FeedPaper()
    Framework.Functions.TriggerCallback('Framework:HasItem', function(HasItem)
        if HasItem then
            ...
            TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items['money-paper'], "remove")
            TriggerServerEvent('qb-illegal:server:add:printer:item', CurrentLab, 'Paper-Count', 1)
        else
            Framework.Functions.Notify('Missing something..', 'error')
        end
    end, "money-paper")
end
...
function FeedInkt()
    Framework.Functions.TriggerCallback('Framework:HasItem', function(HasItem)
        if HasItem then
            ...
            TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items['money-inkt'], "remove")
            TriggerServerEvent('qb-illegal:server:add:printer:item', CurrentLab, 'Inkt-Count', 1)
        else
            Framework.Functions.Notify('Missing something..', 'error')
        end
    end, "money-inkt")
...
function GetTotalMoney()
    if Config.Labs[CurrentLab]['Total-Money'] > 0 then
        TriggerServerEvent('qb-illegal:server:get:money:printer:money')
    end
end
    ...
    while true do
        ...
        if LoggedIn then
            if Config.Labs[3]['Paper-Count'] > 0 and Config.Labs[3]['Inkt-Count'] > 0 then
                TriggerServerEvent('qb-illegal:server:remove:printer:item', CurrentLab, 'Inkt-Count', 1)
                TriggerServerEvent('qb-illegal:server:remove:printer:item', CurrentLab, 'Paper-Count', 1)
                Citizen.Wait(60000)
                TriggerServerEvent('qb-illegal:server:set:printer:money', CurrentLab, math.random(2500, 5000))
                Citizen.Wait(150)
            end
        end
    end
...
RegisterNetEvent('qb-illegal:client:sync:items')
AddEventHandler('qb-illegal:client:sync:items', function(ItemType, ConfigData)
    Config.Labs[3][ItemType] = ConfigData
end)

RegisterNetEvent('qb-illegal:client:sync:money')
AddEventHandler('qb-illegal:client:sync:money', function(ConfigData)
