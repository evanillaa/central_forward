...
Framework.Functions.CreateCallback('qb-illegal:server:get:config', function(source, cb)
    cb(Config)
end)
...
Framework.Functions.CreateCallback('qb-illegal:serverhas:robbery:item', function(source, cb)
...
Framework.Functions.CreateCallback('qb-illegal:server:has:drugs', function(source, cb)
    local src = source
...
Framework.Functions.CreateCallback('qb-illegal:server:get:drugs:items', function(source, cb)
    local src = source
...
RegisterServerEvent('qb-illegal:server:unpack:coke')
AddEventHandler('qb-illegal:server:unpack:coke', function()
    local Player = Framework.Functions.GetPlayer(source)
    if Player.Functions.RemoveItem('packed-coke-brick', 1) then
        Player.Functions.AddItem('pure-coke-brick', math.random(2, 3))
        TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items['pure-coke-brick'], "add")
        TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items['packed-coke-brick'], "remove")
    end
end)

RegisterServerEvent('qb-illegal:server:finish:corner:selling')
AddEventHandler('qb-illegal:server:finish:corner:selling', function(Price, ItemName, ItemAmount)
    local Player = Framework.Functions.GetPlayer(source)
    if Player.Functions.RemoveItem(ItemName, ItemAmount) then
        Player.Functions.AddMoney('cash', Price, 'corner-selling')
        TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items[ItemName], "remove")
    end
end)
                ...
                SetTimeout(SellData['Wait'], function()
                    if Config.Labs[LabId]['Inventory'][Slot] ~= nil then
                        ...
                        if SellData['Success'] == 'coke-powder' then
                            TriggerClientEvent('qb-illegal:client:play:sound:coke', -1)
                        end
                        TriggerClientEvent('qb-illegal:client:sync:inventory', -1, LabId, Config.Labs[LabId]['Inventory'])
                    end
                end)
                if Amount > 1 then
...
RegisterServerEvent('qb-illegal:server:add:ingredient')
AddEventHandler('qb-illegal:server:add:ingredient', function(LabId, IngredientName, Bool, Amount)
  ...
  if Config.Labs[LabId]['Ingredients']['meth-ingredient-1'] and Config.Labs[LabId]['Ingredients']['meth-ingredient-2'] then
    ...
    TriggerClientEvent('qb-illegal:client:start:cooking', -1, LabId)
  end
  TriggerClientEvent('qb-illegal:client:sync:meth', -1, Config.Labs[LabId], LabId, false)
end)

RegisterServerEvent('qb-illegal:server:get:meth')
AddEventHandler('qb-illegal:server:get:meth', function(RandomAmount, LabId)
 local src = source
 local Player = Framework.Functions.GetPlayer(src)
 ResetMethLab(LabId)
 Citizen.SetTimeout(150, function()
    Player.Functions.AddItem('meth-powder', RandomAmount)
    TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['meth-powder'], "add")
 end)
end)

RegisterServerEvent('qb-illegal:server:reset:meth')
AddEventHandler('qb-illegal:server:reset:meth', function(LabId)
    ResetMethLab(LabId)
end)
...
function ResetMethLab(LabId)
 ...
 TriggerClientEvent('qb-illegal:client:sync:meth', -1, Config.Labs[LabId], LabId, true)
end
...
RegisterServerEvent('qb-illegal:server:add:printer:item')
AddEventHandler('qb-illegal:server:add:printer:item', function(LabId, ItemType, Amount)
    Config.Labs[LabId][ItemType] = Config.Labs[LabId][ItemType] + Amount
    TriggerClientEvent('qb-illegal:client:sync:items', -1, ItemType, Config.Labs[LabId][ItemType])
end)

RegisterServerEvent('qb-illegal:server:remove:printer:item')
AddEventHandler('qb-illegal:server:remove:printer:item', function(LabId, ItemType, Amount)
    Config.Labs[LabId][ItemType] = Config.Labs[LabId][ItemType] - Amount
    TriggerClientEvent('qb-illegal:client:sync:items', -1, ItemType, Config.Labs[LabId][ItemType])
end)

RegisterServerEvent('qb-illegal:server:set:printer:money')
AddEventHandler('qb-illegal:server:set:printer:money', function(LabId, Amount)
    Config.Labs[LabId]['Total-Money'] = Config.Labs[LabId]['Total-Money'] + Amount
    TriggerClientEvent('qb-illegal:client:sync:money', -1, Config.Labs[LabId]['Total-Money'])
end)

RegisterServerEvent('qb-illegal:server:get:money:printer:money')
AddEventHandler('qb-illegal:server:get:money:printer:money', function()
    local src = source
    local Player = Framework.Functions.GetPlayer(src)
    Player.Functions.AddMoney('cash', Config.Labs[3]['Total-Money'], 'money-printer')
    Config.Labs[3]['Total-Money'] = 0
    TriggerClientEvent('qb-illegal:client:sync:money', -1, Config.Labs[3]['Total-Money'])
end)

--- Electrnics 

RegisterServerEvent('qb-illegal:server:sell:electrnoics')
AddEventHandler('qb-illegal:server:sell:electrnoics', function()
    local src = source
    local Player = Framework.Functions.GetPlayer(src)
    for k, v in pairs(Player.PlayerData.items) do
        if v.name == 'stolen-tv' then
            ...
            TriggerClientEvent('qb-inventory:client:ItemBox', Player.PlayerData.source, Framework.Shared.Items['stolen-tv'], "remove")
        elseif v.name == 'stolen-micro' then
            Player.Functions.RemoveItem('stolen-micro', 1)
            Player.Functions.AddItem('money-roll', math.random(15, 25))
            Player.Functions.AddMoney('cash', math.random(1500, 2100), 'sold-stolen-goods')
            TriggerClientEvent('qb-inventory:client:ItemBox', Player.PlayerData.source, Framework.Shared.Items['stolen-micro'], "remove")
        elseif v.name == 'stolen-pc' then
            Player.Functions.RemoveItem('stolen-pc', 1)
            Player.Functions.AddItem('money-roll', math.random(15, 25))
            Player.Functions.AddMoney('cash', math.random(1500, 2100), 'sold-stolen-goods')
            TriggerClientEvent('qb-inventory:client:ItemBox', Player.PlayerData.source, Framework.Shared.Items['stolen-pc'], "remove")
        end
    end
end)