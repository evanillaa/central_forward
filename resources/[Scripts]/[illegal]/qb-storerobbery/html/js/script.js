...
StoreRob.Functions.ClosePinpad = function(HasPutin) {
    ...
    $.post('http://qb-storerobbery/PinpadClose', JSON.stringify({
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        ...
        case 13:
            var v = $("#PINbox").val();
            if (v == "") {
                $.post('http://qb-storerobbery/ClickFail', JSON.stringify({}))
                $.post('http://qb-storerobbery/ErrorMessage', JSON.stringify({
                ...
                $("#PINbox").val("");
                $.post('http://qb-storerobbery/Click', JSON.stringify({}))
                $.post('http://qb-storerobbery/EnterPincode', JSON.stringify({
                    pin: data.pin
                }))
