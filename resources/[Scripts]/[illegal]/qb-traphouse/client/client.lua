...
function EnterTrapHouse()
  local TrapHouse = {}
  local PlayerCoords = GetEntityCoords(PlayerPedId())
  local CoordsTable = {x = Config.TrapHouse["Coords"]['Enter']["X"], y = Config.TrapHouse["Coords"]['Enter']["Y"], z = Config.TrapHouse["Coords"]['Enter']["Z"] - Config.TrapHouse["Coords"]['Enter']["Z-OffSet"]-35}
  InSideTrap = true
...
function TryToSell()
    Framework.Functions.TriggerCallback("qb-traphouse:server:has:sell:item", function(HasSellItem)
        if HasSellItem then
            ...
            Framework.Functions.Notify('You have no items to sell..', 'error')
        end
    end)
end
