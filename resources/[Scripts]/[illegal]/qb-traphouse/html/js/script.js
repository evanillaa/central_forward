...
StoreRob.Functions.ClosePinpad = function(HasPutin) {
    ...
    $.post('http://qb-traphouse/PinpadClose', JSON.stringify({
...
$(document).on('keydown', function() {
    switch(event.keyCode) {
        ...
        case 13:
            var v = $("#PINbox").val();
            if (v == "") {
                $.post('http://qb-traphouse/ClickFail', JSON.stringify({}))
                $.post('http://qb-traphouse/ErrorMessage', JSON.stringify({
                ...
                $("#PINbox").val("");
                $.post('http://qb-traphouse/Click', JSON.stringify({}))
                $.post('http://qb-traphouse/EnterPincode', JSON.stringify({
                    pin: data.pin
                }))
