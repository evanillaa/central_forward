...
AddEventHandler('qb-burgershot:server:finish:burger', function(BurgerName)
    ...
    for k, v in pairs(Config.BurgerItems) do
        ...
        TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items[v], "remove")
    end
    Citizen.SetTimeout(350, function()
        Player.Functions.AddItem(BurgerName, 1)
        TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items[BurgerName], "add")
    end)
end)
...
AddEventHandler('qb-burgershot:server:finish:fries', function()
    ...
    if Player.Functions.RemoveItem('burger-potato', 1) then
        TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['burger-potato'], "remove")
        Player.Functions.AddItem('burger-fries', math.random(3, 5))
        TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['burger-fries'], "add")
    end
end)
...
AddEventHandler('qb-burgershot:server:finish:patty', function()
    ...
    if Player.Functions.RemoveItem('burger-raw', 1) then
        TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['burger-raw'], "remove")
        Player.Functions.AddItem('burger-meat', 1)
        TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items['burger-meat'], "add")
    end
end)
...
AddEventHandler('qb-burgershot:server:finish:drink', function(DrinkName)
    ...
    TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items[DrinkName], "add")
end)
    ...
    for k, v in pairs(Framework.Functions.GetPlayers()) do
        ...
        if Player ~= nil then
            if Player.PlayerData.job.name == 'burger' and Player.PlayerData.job.onduty then
                ...
                TriggerClientEvent('qb-inventory:client:ItemBox', Player.PlayerData.source, Framework.Shared.Items['burger-ticket'], "add")
            end
        end
    end
end)
...
AddEventHandler('qb-burgershot:server:sell:tickets', function()
    ...
    TriggerClientEvent('qb-inventory:client:ItemBox', Player.PlayerData.source, Framework.Shared.Items['burger-ticket'], "remove")
end)