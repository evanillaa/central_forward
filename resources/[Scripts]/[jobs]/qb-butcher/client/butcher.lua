...
--     if PlayerJob.name == "butcher" then 
--         while Framework.Functions.GetPlayerData().job == nil do Wait(0) end
...
function packingg(stanowisko)
	...
	Framework.Functions.TriggerCallback('Framework:HasItem', function(result)
		if result then
			...
			Framework.Functions.Progressbar("wash-", "Packing..", 12000, false, true, {
...
function packingg1(stanowisko)
	...
	Framework.Functions.TriggerCallback('Framework:HasItem', function(result)
		if result then
			...
			Framework.TriggerEvent("DoLongHudText", "Keep packing the beef or go to the vehicle and store it.")
			ClearPedTasks(PlayerPedId())
			DeleteEntity(karton)
			DeleteEntity(mieso)
			end, function() -- Cancel
		...
		Framework.TriggerEvent("DoShortHudText", "You have nothing to pack!", 2)
		end
	end, 'slaughteredbeef')
end
...
function portioning(position)
	...
	Framework.Functions.TriggerCallback('Framework:HasItem', function(result)
		...
		if position == 1 then
			...
			Framework.Functions.Progressbar("Cut-", "Slaughtering..", 12000, false, true, {
				disableMovement = true,
				disableCarMovement = true,
				disableMouse = false,
				disableCombat = true,
