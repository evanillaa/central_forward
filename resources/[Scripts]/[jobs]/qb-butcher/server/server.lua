...
AddEventHandler("qb-rawchicken", function()
    ...
    if Player.Functions.GetItemByName('alivechicken') ~= nil and Player.Functions.GetItemByName('alivechicken').amount >= Config.AliveToSlaughter then
		...
		if Framework.Shared.Items["slaughteredchicken"].expire ~= nil then
			info.date = date
			info.expire = Framework.Shared.Items["slaughteredchicken"].expire
		else
			info.quality = 100
		end
...
AddEventHandler("qb-alivechicken", function()
	...
	if Framework.Shared.Items["alivechicken"].expire ~= nil then
		info.date = date
		info.expire = Framework.Shared.Items["alivechicken"].expire
	else
		info.quality = 100
	end
...
AddEventHandler("qb-packchicken", function()
    ...
    if Player.Functions.GetItemByName('slaughteredchicken') ~= nil and Player.Functions.GetItemByName('slaughteredchicken').amount >= 2	then
		...
		if Framework.Shared.Items["packedchicken"].expire ~= nil then
			info.date = date
			info.expire = Framework.Shared.Items["packedchicken"].expire
		else
			info.quality = 100
		end
