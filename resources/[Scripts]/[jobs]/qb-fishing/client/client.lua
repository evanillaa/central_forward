    ...
    while true do
        ...
        if LoggedIn then
            ...
            if Distance <= 2.0 then
                ...
                if IsControlJustReleased(0, 38) then
                    Framework.Functions.TriggerCallback("qb-fishing:server:can:pay", function(DidPay)
                        if DidPay then
                            SpawnFishBoat()
                        end
           ...
           if not IsEntityInWater(PlayerPedId()) then
               ...
               Skillbar.Start({
                   ...
                   if SucceededAttempts + 1 >= NeededAttempts then
                       ...
                       SucceededAttempts = 0
                       --Framework.Functions.TriggerCallback('qb-fishing:server:fish:reward')
               ...
               Framework.Functions.Notify('You are swimming.', 'error')
           end
          else
              Framework.Functions.Notify('You are in a vehicle.', 'error')
          end
       else
           Framework.Functions.Notify('You are not in the fishing area.', 'error')
       end
      end
  end)
end)
...
function SpawnFishBoat()
    ...
    Framework.Functions.SpawnVehicle('dinghy', function(vehicle)
     ...
     Framework.Functions.Notify('You have recieved your boat.', 'success')
