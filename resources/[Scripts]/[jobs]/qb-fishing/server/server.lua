...
Framework.Functions.CreateCallback('qb-fishing:server:can:pay', function(source, cb, price)
...
--Framework.Functions.CreateCallback('qb-fishing:server:fish:reward', function()
    local Player = Framework.Functions.GetPlayer(source)
    ...
    if RandomValue >= 100 and RandomValue < 650 then
        ...
        if SubValue == 1 then
            ...
            TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items['fish-1'], "add") 
        elseif SubValue == 2 then
            Player.Functions.AddItem('fish-2', 1)
            TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items['fish-2'], "add") 
        else
            Player.Functions.AddItem('fish-3', 1)
            TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items['fish-3'], "add") 
        end
    elseif RandomValue >= 700 and RandomValue < 820 then
        local SubValue = math.random(1,2)
        if SubValue == 1 then
            Player.Functions.AddItem('methburn', 1)
            TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items['methburn'], "add")  
        else
            Player.Functions.AddItem('plasticbag', 1)
            TriggerClientEvent('qb-inventory:client:ItemBox', source, Framework.Shared.Items['plasticbag'], "add") 
        end
    else
