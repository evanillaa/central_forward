...
Framework.Functions.CreateCallback('qb-garbagejob:server:HasMoney', function(source, cb)
...
Framework.Functions.CreateCallback('qb-garbagejob:server:CheckBail', function(source, cb)
    local Player = Framework.Functions.GetPlayer(source)
    local CitizenId = Player.PlayerData.citizenid
...
AddEventHandler('qb-garbagejob:server:Pay', function(amount, location)
    ...
    if amount > 0 then
        ...
        if location == #Config.Locations["vuilnisbakken"] then
            ...
            TriggerClientEvent('Framework:Notify', src, "You recieved $"..amount.."", "success")
        end
    else
