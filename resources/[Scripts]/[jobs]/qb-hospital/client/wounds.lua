        ...
        if isLoggedIn then
            if not Config.IsDeath then
               for k, v in pairs(Config.BodyHealth) do
                   ...
                   if v['Pain'] then
                      if TotalPain > 1 then
                        Framework.Functions.Notify("You are in serious pain on several spots..", 'info')
                      else
  ...
  Citizen.SetTimeout(1000, function()
     ...
     Framework.Functions.Progressbar("use_bandage", "Verband omdoen..", 4000, false, true, {
        ...
        disableCarMovement = false,
         ...
         TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items['bandage'], "remove")
    ...
    if Player ~= -1 and Distance < 1.5 then
      if IsTargetDead(GetPlayerServerId(Player)) then
         Framework.Functions.Progressbar("hospital_revive", "Helping Citizen..", RandomTime, false, true, {
             ...
             TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items["health-pack"], "remove")
...
AddEventHandler('qb-hospital:client:use:painkillers', function()
    Citizen.SetTimeout(1000, function()
        ...
        Framework.Functions.Progressbar("use_bandage", "Taking Oxycodons..", 3000, false, true, {
            ...
            TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items["painkillers"], "remove")
            Config.OnOxy = true
         ...
         Framework.Functions.Notify("You still have remains ofoxycodon in your body..", "error")
       end 
    end)
end)
