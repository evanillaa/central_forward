...
AddEventHandler('wood:getItem', function()
	...
	if math.random(0, 100) <= Config.ChanceToGetItem then
		...
		TriggerClientEvent("inventory:client:ItemBox", source, Framework.Shared.Items[randomItem], "add")
			...
			TriggerClientEvent('Framework:Notify', source, 'You can no longer carry full inventory with you', "error")  
		end
	    end
    end
end)
...
AddEventHandler('wood_weed:processweed2', function()
	...
	if Player.Functions.GetItemByName('wood_cut') then
		...
		if chance == 1 or chance == 2 or chance == 3 or chance == 4 or chance == 5 or chance == 6 or chance == 7 or chance == 8 then
			...
			TriggerClientEvent("inventory:client:ItemBox", source, Framework.Shared.Items['wood_cut'], "remove")
			TriggerClientEvent("inventory:client:ItemBox", source, Framework.Shared.Items['wood_proc'], "add")
			TriggerClientEvent('Framework:Notify', src, 'Hout verwerkt', "success")  
		else
			
		end 
	else
		TriggerClientEvent('Framework:Notify', src, 'Je hebt net de juiste items', "error") 
	end
end)
	...
	if Item == nil then
	 ...
	 for k, v in pairs(Config.Prices) do
		...
		if Item.amount > 0 then
			...
			TriggerClientEvent("inventory:client:ItemBox", source, Framework.Shared.Items['wood_proc'], "remove")
			xPlayer.Functions.AddMoney("cash", reward, "sold-pawn-items")
			TriggerClientEvent('Framework:Notify', source, '1x Verkocht', "success")  
			--end
        end
