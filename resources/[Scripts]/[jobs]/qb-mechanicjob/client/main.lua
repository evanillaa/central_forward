...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
    ...
    isLoggedIn = true
    Framework.Functions.TriggerCallback('qb-vehicletuning:server:GetAttachedVehicle', function(plates)
        for k, v in pairs(plates) do
            Config.Plates[k].AttachedVehicle = v.AttachedVehicle
        end
    end)

    Framework.Functions.TriggerCallback('qb-vehicletuning:server:GetDrivingDistances', function(retval)
                                    ...
                                    if IsPedInAnyVehicle(PlayerPedId()) then
                                        if not IsThisModelABicycle(GetEntityModel(veh)) then
                                            ...
                                            if IsControlJustPressed(0, Config.Keys["E"]) then
                                                ...
                                                Wait(500)
                                                DoScreenFadeIn(250)
                                                Framework.Functions.TriggerCallback('qb-vehicletuning:server:SetAttachedVehicle', function(result)
                                                end, veh, k)
                                                -- TriggerServerEvent('qb-vehicletuning:server:SetAttachedVehicle', veh, k)
                                            end
                                        else
                                            Framework.Functions.Notify("No bikes please!", "error")
                                        end
                                    end
                                end
                            end
...
function RepairPart(part)
    ...
    Framework.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        for k, v in pairs(StashItems) do
            if v.name == PartData.item then
                if v.amount >= PartData.costs then
                    Framework.Functions.Progressbar("repair_part", Config.ValuesLabels[part].." is being repaired", math.random(5000, 10000), false, true, {
                    ...
                    Framework.Functions.Notify('Not enough materials in the shop..', 'error')
                end
                break
            else
                Framework.Functions.Notify('Not enough materials in the shop..', 'error')
...
AddEventHandler('qb-vehicletuning:client:RepaireeePart', function(part)
    ...
    Framework.Functions.Notify("The "..Config.ValuesLabels[part].." is repaired!")
end)
                ...
                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, vehpos.x, vehpos.y, vehpos.z, true) < 5.0 then
                    if not IsThisModelABicycle(GetEntityModel(veh)) then
                        ...
                        if VehicleStatus[plate] ~= nil and VehicleStatus[plate][part] ~= nil then
                            ...
                            Framework.Functions.Progressbar("repair_advanced", "Fixing vehicle..", lockpickTime, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
...
function SendStatusMessage(statusList)
    if statusList ~= nil then 
        TriggerEvent('chat:addMessage', {
            ...
            args = {'Voertuig Status', round(statusList["engine"]) .. "/" .. Config.MaxStatusValues["engine"] .. " ("..Framework.Shared.Items["advancedrepairkit"]["label"]..")", round(statusList["body"]) .. "/" .. Config.MaxStatusValues["body"] .. " ("..Framework.Shared.Items[Config.RepairCost["body"]]["label"]..")", round(statusList["radiator"]) .. "/" .. Config.MaxStatusValues["radiator"] .. ".0 ("..Framework.Shared.Items[Config.RepairCost["radiator"]]["label"]..")", round(statusList["axle"]) .. "/" .. Config.MaxStatusValues["axle"] .. ".0 ("..Framework.Shared.Items[Config.RepairCost["axle"]]["label"]..")", round(statusList["brakes"]) .. "/" .. Config.MaxStatusValues["brakes"] .. ".0 ("..Framework.Shared.Items[Config.RepairCost["brakes"]]["label"]..")", round(statusList["clutch"]) .. "/" .. Config.MaxStatusValues["clutch"] .. ".0 ("..Framework.Shared.Items[Config.RepairCost["clutch"]]["label"]..")", round(statusList["fuel"]) .. "/" .. Config.MaxStatusValues["fuel"] .. ".0 ("..Framework.Shared.Items[Config.RepairCost["fuel"]]["label"]..")"}
        })
    end
end
