...
function PullOutVehicle()
    if ownsVan == true then
        Framework.Functions.Notify("You already have a vehicle that belongs to us, bring it back or use it.", "error")
    elseif ownsVan == false then
        coords = Config.Zones.Spawn.Pos
        Framework.Functions.SpawnVehicle('panto', function(veh)
...
function DrawTarget()
    ...
    if DeliveriesCount == 4 then
        Framework.Functions.Notify("All pizzas are delivered!","success")
        RemoveCancelBlip()
      ...
      if pizza == 1 then
        Framework.Functions.Notify("You still have one more delivery to go","inform")
      else
        if pizza == 4 then
          pizza = 'Four'
        elseif pizza == 3 then
        ...
        Framework.Functions.Notify("You have "..pizza.." pizza deliverys to go","success")
      end
        if LastGoal == RandomPoint then
		    ...
		    Framework.Functions.Notify("Deliver the pizza to the costumer","success")
        end
    end
end
            ...
            if Delivered then
                if DistanceFromEndZone <= 2.5 then
                    ...
                    if DistanceFromEndZone <= 7 then
                        if IsControlJustReleased(0, 46) then
                            Framework.Functions.Notify("Nice one take some time to rest !", "success")
                            EndOfWork()
                        end
                    end
                end
                ...
                if DistanceFromCancelZone <= 2.5 then
                    ...
                    if DistanceFromCancelZone <= 7 then
                        if IsControlJustReleased(0, 46) then
                            Framework.Functions.Notify("You refused new orders", "error")
							EndOfWork()
                        end
                    end
                end
...
function EndOfWork()
    ...
    if IsPedInAnyVehicle(ped, false) then
        ...
        if IsVehicleModel(Van, GetHashKey('panto')) then
            Framework.Functions.DeleteVehicle(Van)
            ...
            Framework.Functions.Notify("You need to return to your vehicle", "error")
            Framework.Functions.Notify("If you have lost the company vehicle deliver it on foot!", "error")
        end
    else
        InService = false
        BlipSell = nil
