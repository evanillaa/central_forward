		  			...
		  			if not SilentWeapon then
						if not GunShotAlert then
							...
							TriggerServerEvent('qb-hud:server:gain:stress', math.random(1,3))
		  					TriggerServerEvent('qb-police:server:send:alert:gunshots', GetEntityCoords(PlayerPedId()), GunCategory, Framework.Functions.GetStreetLabel(), IsPedInAnyVehicle(PlayerPedId()))
