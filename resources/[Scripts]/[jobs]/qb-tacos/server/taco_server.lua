...
AddEventHandler('qb-taco:server:start:black', function()
    ...
    TriggerClientEvent('qb-inventory:client:ItemBox', Player.PlayerData.source, Framework.Shared.Items['taco-bag'], 'add')
    TriggerClientEvent("hud:client:OnMoneyChange")
end)

Framework.Functions.CreateCallback('qb-tacos:server:GetConfig', function(source, cb)
    cb(Config)
end)

RegisterServerEvent('qb-taco:server:reward:money')
AddEventHandler('qb-taco:server:reward:money', function()
--Framework.Functions.CreateCallback('qb-taco:server:reward:money', function(source, cb)
    local Player = Framework.Functions.GetPlayer(source)
    Player.Functions.AddMoney('cash', math.random(50, 312), "taco-shop-reward")
    TriggerClientEvent('Framework:Notify', source, "Taco delivered! Go back to the taco shop for a new delivery")
    Player.Functions.RemoveItem('taco-bag', 1)
    TriggerClientEvent('qb-inventory:client:ItemBox', Player.PlayerData.source, Framework.Shared.Items['taco-bag'], 'remove')
...
AddEventHandler('qb-tacos:server:get:stuff', function()
    ...
    if Player ~= nil then
        ...
        TriggerClientEvent('qb-inventory:client:ItemBox', Player.PlayerData.source, Framework.Shared.Items['taco-box'], 'add')
    end
end)
...
AddEventHandler('qb-tacos:server:rem:taco', function()
    ...
    if Player ~= nil then
        ...
        TriggerClientEvent('qb-inventory:client:ItemBox', Player.PlayerData.source, Framework.Shared.Items['taco'], 'add')
    end
end)
...
AddEventHandler('qb-tacos:server:rem:stuff', function(what)
   ...
   -- if Player ~= nil then
    if Player ~= nil and what == "meat" or what == "lettuce" or what == "taco-box" or what == "taco" then
        ...
        TriggerClientEvent('qb-inventory:client:ItemBox', Player.PlayerData.source, Framework.Shared.Items['what'], 'add')
    end
end)
...
Framework.Functions.CreateCallback('qb-taco:server:get:ingredient', function(source, cb)
    local src = source
...
Framework.Functions.CreateCallback('qb-taco:server:get:tacobox', function(source, cb)
    local src = source
    local Ply = Framework.Functions.GetPlayer(src)
...
Framework.Functions.CreateCallback('qb-taco:server:get:tacos', function(source, cb)
    local src = source
    local Ply = Framework.Functions.GetPlayer(src)
