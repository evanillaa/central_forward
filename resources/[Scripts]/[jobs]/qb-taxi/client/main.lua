    ...
    if whitelistedVehicle() then
        if NpcData.CountDown == nil then
            if not NpcData.Active then
                ...
                Framework.Functions.Notify('The customer is assigned to your gps.', 'success')
                        ...
                        if dist < 20 then
                            ...
                            if dist < 5 then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    Framework.Functions.Notify('Deliver the customer to their location')
                                    if NpcData.NpcBlip ~= nil then
                                        RemoveBlip(NpcData.NpcBlip)
                                    end
                                    GetDeliveryLocation()
                ...
                Framework.Functions.Notify('You are already driving a customer around')
            end
        else
            Framework.Functions.Notify('No customers available')
        end
    else
        Framework.Functions.Notify('You are not in a taxicab')
    end
end)
            ...
            if dist < 10 then
                ...
                if dist < 3 then
                    ...
                    if IsControlJustPressed(0, Keys["E"]) then
                        ...
                        Framework.Functions.TriggerCallback('qb-taxi:server:NpcPay', function(result)
                        end, meterData.currentFare)
                        
                        Framework.Functions.Notify('Costumer has arrived at its location. Great work!', 'success')
                        if NpcData.DeliveryBlip ~= nil then
                            RemoveBlip(NpcData.DeliveryBlip)
                        end
...
AddEventHandler('qb-taxi:client:toggleMeter', function()
    ...
    if IsPedInAnyVehicle(ped, false) then
        if whitelistedVehicle() then
            ...
            Framework.Functions.Notify('This vehicle is not equiped with a taxi meter', 'error')
        end
    else
...
AddEventHandler('qb-taxi:client:enableMeter', function()
    ...
    if meterIsOpen then
        ...
        Framework.Functions.Notify('The meter is not active.', 'error')
    end
end)
