...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1000, function()
     ...
     Citizen.Wait(150)
     Framework.Functions.TriggerCallback("qb-tow:server:get:config", function(config)
        Config = config
     end)
                ...
                local Distance = GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, Config.JobLocations['Flatbed']['X'], Config.JobLocations['Flatbed']['Y'], Config.JobLocations['Flatbed']['Z'], true)
                  if Distance <= 6.0 then
                   ...
                   if IsPedSittingInAnyVehicle(PlayerPedId()) then
                     ...
                     if IsControlJustReleased(0, 38) then
                       Framework.Functions.TriggerCallback("qb-tow:server:do:bail", function(DidBail)
                           if DidBail then
                               GetTowVehicle()
                           else
                             Framework.Functions.Notify('Je hebt niet genoeg contant geld..', 'error', 5500)
                           end
                        end)
                     end
                  end
...
AddEventHandler('qb-tow:client:toggle:npc', function()
   if not Config.IsDoingNpc then
     ...
     Framework.Functions.Notify('Je bent begonnen met een takel opdracht rij naar de locatie om het voertuig op te halen..', 'success', 5500)
   else
    if Config.CurrentTowedVehicle ~= nil then
        Framework.Functions.Notify('Je bent nog bezig met een voertuig..', 'error', 5500)
    else
        DeleteWaypoint()
        RemoveBlip(JobBlip)
        ResetAll()
        Framework.Functions.Notify('Je bent gestopt met je takel opdracht..', 'error', 5500)
    end
   end
end)
...
AddEventHandler('qb-tow:client:hook:car', function()
    ...
    if IsVehicleValid(TowVehicle) then
      if Config.IsDoingNpc then
               ...
               if Vehicle ~= TowVehicle then
                   if GetHashKey(Config.Locations[Config.CurrentNpc]['Model']) == GetEntityModel(Vehicle) then
                       Framework.Functions.Progressbar("towing-vehicle", "Voertuig erop zetten..", 5000, false, true, {
                           ...
                           Framework.Functions.Notify("Voertuig succesvol opgetakelt, breng hem nu naar Harold's Depot", "success")
                       end, function() -- Cancel
                        ...
                        Framework.Functions.Notify('Dit is niet het juiste voertuig..', 'error', 5500)
                   end
               else
                   Framework.Functions.Notify('Je kan niet je flatbed op je flatbed zetten?!?', 'error', 5500)
               end
            else
                Framework.Functions.Notify('Geen voertuig?', 'error', 5500)
            end
        else
            Framework.Functions.Progressbar("untowing_vehicle", "Voertuig eraf halen..", 5000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                if exports['qb-garages']:IsNearDepot() then
                    Framework.Functions.Notify("Voertuig ingeleverd!", 'success')
                ...
                Config.CurrentTowedVehicle = nil
                Framework.Functions.Notify("Voertuig eraf gehaald!")
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_ped", 1.0)
                Framework.Functions.Notify("Mislukt!", "error")
            end)
        ...
        Framework.Functions.Notify('Je zat niet in een flatbed', 'error', 5500)
    end
end)
