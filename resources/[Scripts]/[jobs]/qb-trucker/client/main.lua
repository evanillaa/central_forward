                        ...
                        if IsControlJustReleased(0, Keys["E"]) then
                            if IsPedInAnyVehicle(PlayerPedId(), false) then
                                if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
                                    ...
                                    Framework.Functions.Notify('You need to be the driver..')
                                end
                            else
                                MenuGarage()
                            ...
                            if not hasBox then
                                ...
                                if isTruckerVehicle(vehicle) and CurrentPlate == GetVehicleNumberPlateText(vehicle) then
                                    ...
                                    if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, trunkpos.x, trunkpos.y, trunkpos.z, true) < 1.5 and not isWorking then
                                        ...
                                        if IsControlJustReleased(0, Keys["E"]) then
                                            isWorking = true
                                            Framework.Functions.Progressbar("work_carrybox", "Picking products..", 2000, false, true, {
                                                ...
                                                isWorking = false
                                                StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
                                                Framework.Functions.Notify("Canceld..", "error")
                                            end)
                                        end
                                    else
                            ...
                            elseif hasBox then
                                if GetDistanceBetweenCoords(pos.x, pos.y, pos.z, CurrentLocation.x, CurrentLocation.y, CurrentLocation.z, true) < 1.5 and not isWorking then
                                    ...
                                    if IsControlJustReleased(0, Keys["E"]) then
                                        ...
                                        Framework.Functions.Progressbar("work_dropbox", "Delivering products..", 2000, false, true, {
                                            disableMovement = true,
                                            disableCarMovement = true,
                                            disableMouse = false,
                                            disableCombat = true,
                                            ...
                                            if currentCount == CurrentLocation.dropcount then
                                                ...
                                                Framework.Functions.Notify("You delivered all products, on to the next point.")
                                                if CurrentBlip ~= nil then
                                                    RemoveBlip(CurrentBlip)
                                                    CurrentBlip = nil
                                                end
...
function getNewLocation()
    ...
    if location ~= 0 then
        ...
        Framework.Functions.Notify("You went by all stores.. Time for your pay check!")
        if CurrentBlip ~= nil then
            RemoveBlip(CurrentBlip)
            CurrentBlip = nil
        end
