				...
				if GetDistanceBetweenCoords(coords, Config.Startpoint.x, Config.Startpoint.y, Config.Startpoint.z, true) < Config.Startpoint.d and not timeout then
					...
					if IsControlJustPressed(1, 38) then
						...
						Citizen.CreateThread(function()
							...
							Framework.Functions.Notify("Deliver this to our client, and dont fuck the car up!", "success", 7000)
							timeout = true
...
AddEventHandler('qb-cardealer:job:sellveh', function()
	...
	elseif health <= 1600 then
		...
		Framework.Functions.Notify("Thanks for bringing me my vehicle, enjoy your pay", "success", 2500)
		TriggerServerEvent('qb-cardealer:job:pay', type)
	elseif health <= 1400 then
		local type = 'shitpay'
		Framework.Functions.Notify("The vehicle looks kinda broken, heres your payment, next time dont fuck it up", "success", 2500)
		TriggerServerEvent('qb-cardealer:job:pay', type)
	elseif health <= 800 then
		local type = 'Lpay'
	elseif health == 0 then
		Framework.Functions.Notify("Are you serious, the vehicle is broken, no payment for you, asshole", "success", 2500)
		TriggerServerEvent('qb-cardealer:job:pay', type)
	end
end)