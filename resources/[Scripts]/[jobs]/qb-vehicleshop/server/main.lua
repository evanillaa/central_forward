...
AddEventHandler('qb-vehicleshop:server:buyVehicle', function(vehicleData, garage)
    ...
    if (balance - vData["price"]) >= 0 then
        ...
        Framework.Functions.ExecuteSql(false, "INSERT INTO `characters_vehicles` (`citizenid`, `vehicle`, `plate`, `garage`, `state`, `mods`, `metadata`) VALUES ('"..pData.PlayerData.citizenid.."', '"..vData["model"].."', '"..plate.."', 'Rode Parking', 'out', '{}', '"..json.encode(VehicleMeta).."')")
		...
		TriggerClientEvent("Framework:Notify", src, "You dont have enough cash. You are missing: €"..format_thousand(vData["price"] - balance), "error", 5000)
    end
end)
...
AddEventHandler('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle, class)
    ...
    if (balance - vehiclePrice) >= 0 then
        ...
        Framework.Functions.ExecuteSql(false, "INSERT INTO `characters_vehicles` (`citizenid`, `vehicle`, `plate`, `garage`, `state`, `mods`, `metadata`) VALUES ('"..pData.PlayerData.citizenid.."', '"..vehicle.."', '"..plate.."', 'Rode Parking', 'out', '{}', '"..json.encode(VehicleMeta).."')")
 
        TriggerClientEvent("Framework:Notify", src, "Thank you! your vehicle will be with you shortly.", "success", 5000)
        TriggerClientEvent('qb-vehicleshop:client:buyShowroomVehicle', src, vehicle, plate)
        pData.Functions.RemoveMoney('bank', vehiclePrice, "vehicle-bought-in-showroom")
    else
        TriggerClientEvent("Framework:Notify", src, "You dont have enough funds on your bank account. You are missing: €"..format_thousand(vehiclePrice - balance), "error", 5000)
    end
end)
...
Framework.Commands.Add("sellv", "Sell vehicle from Luxery", {}, false, function(source, args)
    ...
    if Player.PlayerData.job.name == "cardealer" then
        if TargetId ~= nil then
            ...
            TriggerClientEvent('Framework:Notify', source, 'You need to enter an ID!', 'error')
        end
    else
...
Framework.Commands.Add("testrit", "Do a testdrive", {}, false, function(source, args)
    local Player = Framework.Functions.GetPlayer(source)
    local TargetId = args[1]
...
AddEventHandler('qb-vehicleshop:server:ConfirmVehicle', function(ShowroomVehicle)
    ...
    if Player.PlayerData.money.cash >= VehPrice then
        ...
        Framework.Functions.ExecuteSql(false, "INSERT INTO `characters_vehicles` (`citizenid`, `vehicle`, `plate`, `garage`, `state`, `mods`, `metadata`) VALUES ('"..pData.PlayerData.citizenid.."', '"..ShowroomVehicle.."', '"..plate.."', 'Rode Parking', 'out', '{}', '"..json.encode(VehicleMeta).."')")
    elseif Player.PlayerData.money.bank >= VehPrice then
        ...
        if Player.PlayerData.money.cash > Player.PlayerData.money.bank then
            TriggerClientEvent('Framework:Notify', src, 'You dont have enough cash. You are missing: ('..(Player.PlayerData.money.cash - VehPrice)..',-)')
        else
            TriggerClientEvent('Framework:Notify', src, 'You dont have enough funds on the bank account. You are missing: ('..(Player.PlayerData.money.bank - VehPrice)..',-)')
        end
    end
end)

Framework.Functions.CreateCallback('qb-vehicleshop:server:SellVehicle', function(source, cb, vehicle, plate)
    local VehicleData = Framework.Shared.Vehicles[vehicle]
    local src = source
    local Player = Framework.Functions.GetPlayer(src)
