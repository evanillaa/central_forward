...
QBCoreAdmin = {}
QBCoreAdmin.Functions = {}
in_noclip_mode = false

QBCoreAdmin.Functions.DrawText3D = function(x, y, z, text, lines)
    if lines == nil then
        lines = 1
    end
...
Citizen.CreateThread(function()
    ...
    WarMenu.CreateMenu('admin', 'QBCore Admin')
    ...
    while true do
		...
		--end
            ...
            if WarMenu.MenuButton('Hotwire Vehicle', 'vehOptions') then
                ...
                exports['qb-vehiclekeys']:SetVehicleKey(GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId())), true)
            end
            WarMenu.Display()
        ...
        if myPermissionRank == "god" then
            if WarMenu.MenuButton('Fill Fuel', 'vehOptions') then
                local vehicle = (GetVehiclePedIsIn(PlayerPedId()))
                    exports['qb-fuel']:SetFuelLevel(vehicle, GetVehicleNumberPlateText(vehicle), 99, false)
            end
        ...
        elseif WarMenu.IsMenuOpened('playerOptions') then
            if WarMenu.MenuButton('Crash', currentPlayer) then
                TriggerServerEvent('qb-admin:server:crash', GetPlayerServerId(currentPlayer))
            end
            ...
            if WarMenu.MenuButton('Revive', currentPlayer) then
                ...
                TriggerEvent('qb-hospital:client:revive')
            end
            ...
            --     TriggerServerEvent('qb-admin:server:give:clothing', currentPlayer)
            -- end
        ...
        elseif WarMenu.IsMenuOpened('adminOptions') then
            if WarMenu.ComboBox('Ban Length', bans, currentBanIndex, selectedBanIndex, function(currentIndex, selectedIndex)
                ...
                if reason ~= nil and reason ~= "" and time ~= 0 then
                    local target = GetPlayerServerId(currentPlayer)
                    TriggerServerEvent("qb-admin:server:banPlayer", target, reason)
                end
            end
...
function OpenTargetInventory(targetId)
    WarMenu.CloseMenu()
    TriggerServerEvent('qb-admin:server:open:inventory', targetId)
end
...
RegisterNetEvent('qb-admin:client:give:clothing')
AddEventHandler('qb-admin:client:give:clothing', function(TargetId)
  TriggerServerEvent("qb-admin:server:give:clothing", TargetId)
end)
    ...
    while true do
        ...
        if showNames then
            for _, player in pairs(GetPlayersFromCoords(GetEntityCoords(PlayerPedId()), 25.0)) do
                ...
                QBCoreAdmin.Functions.DrawText3D(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z + 1.0, '['..PlayerId..'] '..PlayerName)
            end
        else
            Citizen.Wait(1000)
        end
	...
	while true do
        ...
        if deleteLazer then
            ...
            if hit and (IsEntityAVehicle(entity) or IsEntityAPed(entity) or IsEntityAnObject(entity)) then
                ...
                QBCoreAdmin.Functions.DrawText3D(entityCoord.x, entityCoord.y, entityCoord.z, "Object: " .. entity .. " Model: " .. GetEntityModel(entity).. " \nPress [~g~E~s~] to delete this object.", 2)
...
AddEventHandler('qb-admin:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if Framework.Shared.Vehicles[hash] ~= nil and next(Framework.Shared.Vehicles[hash]) ~= nil then
            TriggerServerEvent('qb-admin:server:SaveCar', props, Framework.Shared.Vehicles[hash], GetHashKey(veh), plate)
        else
...
RegisterNetEvent("qb-admin:cleanup:delallveh")
AddEventHandler("qb-admin:cleanup:delallveh", function ()
...
RegisterNetEvent("qb-admin:cleanup:objectwipe")
AddEventHandler("qb-admin:cleanup:objectwipe", function ()
    local totalobj = 0
