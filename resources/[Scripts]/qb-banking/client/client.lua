...
RegisterNetEvent('qb-banking:client:open:bank')
AddEventHandler('qb-banking:client:open:bank', function()
    Citizen.SetTimeout(450, function()
        OpenBank(true)
    end)
end)

RegisterNetEvent('qb-banking:client:open:atm')
AddEventHandler('qb-banking:client:open:atm', function()
    Citizen.SetTimeout(450, function()
        OpenBank(false)
    end)
end)
...
RegisterNUICallback('ClickSound', function(data)
    ...
    elseif data.success == 'click' then
     ...
     TriggerEvent("qb-sound:client:play", data.success, 0.25)
    end
end)

RegisterNUICallback('Withdraw', function(data)
    if IsNearAnyBank() or IsNearAtm() then
      TriggerServerEvent('qb-banking:server:withdraw', data.RemoveAmount, data.BankId) 
    end
end)

RegisterNUICallback('Deposit', function(data)
    if IsNearAnyBank() then
      TriggerServerEvent('qb-banking:server:deposit', data.AddAmount, data.BankId) 
    end
end)

RegisterNUICallback('CreateAccount', function(data)
     if IsNearAnyBank() or IsNearAtm() then
       TriggerServerEvent('qb-banking:server:create:account', data.Name, data.Type)
    end
end)

RegisterNUICallback('AddUserToAccount', function(data)
     if IsNearAnyBank() or IsNearAtm() then
       TriggerServerEvent('qb-banking:server:add:user', data.BankId, data.TargetBsn)
    end
end)

RegisterNUICallback('DeleteFromAccount', function(data)
     if IsNearAnyBank() or IsNearAtm() then
       TriggerServerEvent('qb-banking:server:remove:user', data.BankId, data.TargetBsn)
     end
end)

RegisterNUICallback('DeleteAccount', function(data)
    if IsNearAnyBank() or IsNearAtm() then
        TriggerServerEvent('qb-banking:server:remove:account', data.BankId)
      end
end)

RegisterNUICallback('GetTransactions', function(data)
    if IsNearAnyBank() or IsNearAtm() then
        Framework.Functions.TriggerCallback('qb-banking:server:get:account:transactions', function(Transactions)
         SendNUIMessage({
...
RegisterNUICallback('GetSharedAccounts', function(data, cb)
    Framework.Functions.TriggerCallback('qb-banking:server:get:shared:account', function(Accounts)
        cb(Accounts)
    end)  
end)

RegisterNUICallback('GetPrivateAcounts', function(data, cb)
    Framework.Functions.TriggerCallback('qb-banking:server:get:private:account', function(Accounts)
        cb(Accounts)
    end)  
end)
...
RegisterNUICallback('GetAccountUsers', function(data)
    Framework.Functions.TriggerCallback('qb-banking:server:get:account:users', function(Accounts)
        SendNUIMessage({
          action = 'SetupUsers',
          accounts = Accounts,
...
RegisterNetEvent('qb-banking:client:check:players:near')
AddEventHandler('qb-banking:client:check:players:near', function(TargetPlayer, Amount)
    local Player, Distance = Framework.Functions.GetClosestPlayer()
    if Player ~= -1 and Distance < 3.0 then
        if GetPlayerServerId(Player) == TargetPlayer then
            exports['qb-assets']:RequestAnimationDict('friends@laf@ig_5')
            TaskPlayAnim(PlayerPedId(), 'friends@laf@ig_5', 'nephew', 5.0, 1.0, 5.0,48, 0.0, 0, 0, 0)
            TriggerServerEvent('qb-banking:server:give:cash', TargetPlayer, Amount) 
        else
...
function OpenBank(CanDeposit, UseAnim)
    exports['qb-assets']:RequestAnimationDict('amb@prop_human_atm@male@idle_a')
    exports['qb-assets']:RequestAnimationDict('amb@prop_human_atm@male@exit')
    TaskPlayAnim(PlayerPedId(), "amb@prop_human_atm@male@idle_a", "idle_b", 1.0, 1.0, -1, 49, 0, 0, 0, 0 )
    Framework.Functions.Progressbar("bank", "Injecting Card..", 4500, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
