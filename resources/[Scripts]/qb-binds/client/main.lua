...
AddEventHandler('qb-binds:client:openUI', function()
    openBindingMenu()
end)
                ...
                if IsControlJustPressed(0, Keys[v]) or IsDisabledControlJustPressed(0, Keys[v]) then
                    ...
                    if next(keyMeta) ~= nil then
                        if keyMeta[v]["command"] ~= "" then
                            ...
                            Framework.Functions.Notify('You have nothing binded to ['..v..'], use /binds to start binding', 'primary', 4000)
                        end
                    else
                        Framework.Functions.Notify('You have nothing binded yet, use /binds to start binding', 'primary', 4000)
                    end
                end
            end
            if keyPressed then
...
RegisterNUICallback('save', function(data)
    ...
    Framework.Functions.Notify('Commandbindings saved!', 'success')
    TriggerServerEvent('qb-binds:server:setKeyMeta', keyData)
end)