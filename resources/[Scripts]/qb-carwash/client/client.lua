...
RegisterNetEvent('qb-carwash:client:sync:water')
AddEventHandler('qb-carwash:client:sync:water', function(WaterId)
    StartWashParticle(WaterId)
end)

RegisterNetEvent('qb-carwash:client:stop:water')
AddEventHandler('qb-carwash:client:stop:water', function(WaterId)
...
RegisterNetEvent('qb-carwash:client:set:busy')
AddEventHandler('qb-carwash:client:set:busy', function(CarWashId, bool)
 Config.CarWashLocations[CarWashId]['Busy'] = bool
end)

RegisterNetEvent('qb-carwash:client:sync:wash')
AddEventHandler('qb-carwash:client:sync:wash', function(Vehicle)
                 ...
                 if Gebied <= 1.5 then
                     ...
                     if IsControlJustReleased(0, Config.Keys['G']) then
                        if not Config.CarWashLocations[k]['Busy'] then
                            Framework.Functions.TriggerCallback("qb-carwash:server:can:wash", function(CanWash)
                            if CanWash then
...
function StartCarWashing(CarWashId, Vehicle)
 ...
 TriggerServerEvent('qb-carwash:server:set:busy', CarWashId, true) 
 SetEveryoneIgnorePlayer(PlayerPedId(), true)
 SetPlayerControl(PlayerPedId(), false)
 Wait(250)
 TriggerServerEvent('qb-carwash:server:sync:water', Config.CarWashLocations[CarWashId]['Particle'])
 ...
 TriggerServerEvent('qb-carwash:server:sync:wash', Vehicle)
 Citizen.Wait(Config.CarWashLocations[CarWashId]['Wait'] / 2)
 TriggerServerEvent('qb-carwash:server:stop:water', Config.CarWashLocations[CarWashId]['Particle'])
 SetPlayerControl(PlayerPedId(), true)
 TriggerServerEvent('qb-carwash:server:set:busy', CarWashId, false) 
end
