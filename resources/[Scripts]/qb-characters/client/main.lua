...
Citizen.CreateThread(function()
	while true do
		...
		if NetworkIsSessionStarted() then
			TriggerEvent('qb-characters:client:chooseChar')
			return
		end
	end
end)
...
RegisterNUICallback('selectCharacter', function(data)
    ...
    DoScreenFadeOut(100)
    TriggerServerEvent('qb-characters:server:loadUserData', cData)
    openCharMenu(false)
    SetEntityAsMissionEntity(charPed, true, true)
    DeleteEntity(charPed)
end)

RegisterNetEvent('qb-characters:client:closeNUI')
AddEventHandler('qb-characters:client:closeNUI', function()
    SetNuiFocus(false, false)
end)

local Countdown = 1

RegisterNetEvent('qb-characters:client:chooseChar')
AddEventHandler('qb-characters:client:chooseChar', function()
    SetNuiFocus(false, false)
    DoScreenFadeOut(10)
    Citizen.Wait(1000)
...
RegisterNUICallback('cDataPed', function(data)
    ...
    DeleteEntity(charPed)
    if cData ~= nil then
        Framework.Functions.TriggerCallback('qb-characters:server:getSkin', function(model, data)
...
RegisterNUICallback('setupCharacters', function()
    Framework.Functions.TriggerCallback("qb-characters:server:get:char:data", function(result)
        SendNUIMessage({
