...
Framework.Commands.Add("logout", "Ga naar het karakter menu.", {}, false, function(source, args)
    ...
    Citizen.Wait(650)
    TriggerClientEvent('qb-characters:client:chooseChar', src)
end, "admin")

RegisterServerEvent('qb-characters:server:loadUserData')
AddEventHandler('qb-characters:server:loadUserData', function(cData)
    local src = source
    if Framework.Player.Login(src, false, cData.citizenid) then
        ...
        TriggerEvent("qb-logs:server:SendLog", "joinleave", "Loaded", "green", "**".. GetPlayerName(src) .. "** ("..cData.citizenid.." | "..src..") loaded..")
	end
end)
...
AddEventHandler('qb-characters:server:createCharacter', function(data)
    ...
    if Framework.Player.Login(src, true, false, newData) then
        print('^2[qb-CORE]^7 '..GetPlayerName(src)..' has succesfully created their char!')
        Framework.Commands.Refresh(src)
        GiveStarterItems(src)
        TriggerClientEvent('qb-spawn:client:choose:appartment', src)
        TriggerClientEvent("qb-characters:client:closeNUI", src)
	end
end)
...
Framework.Functions.CreateCallback("qb-characters:server:get:char:data", function(source, cb)
...
Framework.Functions.CreateCallback("qb-characters:server:getSkin", function(source, cb, cid)
    local src = source
    Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_skins` WHERE `citizenid` = '"..cid.."'", function(result)
