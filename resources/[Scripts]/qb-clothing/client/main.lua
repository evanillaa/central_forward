            ...
            for k, v in pairs(Config.Stores) do
                ...
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 0.15, '~g~E~w~ - Clothingshop')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 0.15, '~g~E~w~ - Barbershop')
                            end
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                ...
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    Framework.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
...
RegisterNUICallback('saveOutfit', function(data, cb)
    ...
    TriggerServerEvent('qb-clothing:saveOutfit', data.outfitName, model, skinData)
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    TriggerEvent('chatMessage', "SYSTEM", "warning", "You have removed "..data.outfitName.."!")
end)
...
RegisterNetEvent('qb-clothing:client:CreateFirstCharacter')
AddEventHandler('qb-clothing:client:CreateFirstCharacter', function()
...
RegisterNetEvent("qb-clothing:loadSkin")
AddEventHandler("qb-clothing:loadSkin", function(new, model, data)
