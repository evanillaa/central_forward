...
Citizen.CreateThread(function()
	while Framework == nil do TriggerEvent('Framework:GetObject', function(obj) Framework = obj end) Wait(0) end
    Framework.Functions.TriggerCallback('coke:processcoords', function(servercoords)
...
Citizen.CreateThread(function()
	while Framework == nil do TriggerEvent('Framework:GetObject', function(obj) Framework = obj end) Wait(0) end
    Framework.Functions.TriggerCallback('coke:startcoords', function(servercoords)
        coord = servercoords
	end)
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if not inUse then
			if dist <= 2.0 then
				sleep = 5
				DrawText3D(coord.x, coord.y, coord.z, 'Press ~g~[ E ]~w~ to rent a Plane for 1500')
				if IsControlJustPressed(1, 51) then
					Framework.Functions.TriggerCallback('coke:pay', function(success)
						if success then
							main()
						end
					end)
...
AddEventHandler('coke:onUse', function()
	if Config.useMythic then
		Framework.Functions.Notify("You used Coke", "success")
	end
	...
	DoScreenFadeIn(2000)
	if Config.useMythic then
		Framework.Functions.Notify("You are feeling normal now..", "success")
	end
...
function main()
	...
	if Config.useMythic then
		Framework.Functions.Notify("Go to the airfield.", "success")
	end
	...
	while checkPlane do
		...
		if DoesEntityExist(airplane) then
			if GetVehicleEngineHealth(airplane) < 0 then
				if Config.useMythic then
					Framework.Functions.Notify("Failed, your plane was Destroyed", "error")
				end
				TriggerServerEvent('coke:updateTable', false)
				RemoveBlip(blip)
...
function delivery()
	if Config.useMythic then
		Framework.Functions.Notify("Get in the plane and pick up the delivery marked on your GPS", "success")
	end
		...
		while delivering do
			...
			if disttocoord <= 30 then
				...
				if IsControlJustPressed(1, 51) then
					...
					Framework.Functions.Progressbar("picking_", "Picking up the delivery..", lockpickTime, false, true, {
					...
					Citizen.Wait(2000)
					Framework.Functions.Notify("Picked up the delivery. Return to the airfield marked on your GPS.", "success")
					Citizen.Wait(2000)
					final()
				end
			else
...
function final()
	Framework.Functions.Notify("Deliver the plane back to a hangar", "success")
		...
		while hangar do
			...
			if IsPedInAnyPlane(GetPlayerPed(-1)) and disttocoord <= 10 then
				...
				if IsControlJustPressed(1, 51) then
					...
					Framework.Functions.Progressbar("lockpick_vehicledoor", "Lockicking..", 1000, false, true, {
						disableMovement = true,
						disableCarMovement = true,
						disableMouse = false,
						disableCombat = true,
			...
			if IsControlJustPressed(1, 51) then		
				...
				Framework.Functions.TriggerCallback('Framework:HasItem', function(result)
					if result then
						...
						Framework.Functions.Notify("You need coke brick to do this", "error")
						isProcessing = false
					end
				end, 'coke_brick')
			end
...
function processing()
	...
	playAnim("anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 40000)

	Framework.Functions.Progressbar("coke-", "Breaking down the coke..", 30000, false, true, {
		disableMovement = true,
		disableCarMovement = true,
		disableMouse = false,
		disableCombat = true,
		...
		TriggerServerEvent('coke:processeda')	
		isProcessing = false
	end, function() -- Cancel
		isProcessing = false
