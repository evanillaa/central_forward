...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
    Citizen.SetTimeout(1250, function()
        ...
        Citizen.Wait(250)
		Framework.Functions.TriggerCallback('qb-crafting:server:get:config', function(ConfigData)
		...
		if LoggedIn then
			...
			if Distance < 2 then
				...
				if IsControlJustReleased(0, 38) then
					...
					TriggerServerEvent("qb-inventory:server:OpenInventory", "crafting_weapon", math.random(1, 99), Crating)
				end
			end
		    ...
		    if CraftObject ~= 0 then
		    	...
		    	if GetDistanceBetweenCoords(PlayerCoords.x, PlayerCoords.y, PlayerCoords.z, ObjectCoords.x, ObjectCoords.y, ObjectCoords.z, true) < 1.5 then
		    		...
		    		if IsControlJustReleased(0, 38) then
						...
						TriggerServerEvent('qb-inventory:server:set:inventory:disabled', true)
		    			TriggerServerEvent("qb-inventory:server:OpenInventory", "crafting", math.random(1, 99), Crating)
		    		end
		    	end
		    end
