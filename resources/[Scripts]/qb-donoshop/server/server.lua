...
Framework.Functions.CreateCallback('qb-vehicleshop:server:get:config', function(source, cb)
    cb(Config)
end)

Framework.Functions.CreateCallback('qb-vehicleshop:server:check:owner', function(source, cb, plate)
...
RegisterNetEvent('qb-vehicleshop:server:sell:vehicle')
AddEventHandler('qb-vehicleshop:server:sell:vehicle', function(Plate, Price)
...
Framework.Commands.Add("refreshcars", "Refresh de car dealer", {}, false, function(source, args)
    ...
    Citizen.SetTimeout(1500, function()
        ...
        TriggerClientEvent('qb-vehicleshop:client:set:vehicles', -1, RandomSport, RandomSedan, RandomMotor, RandomMuscle, RandomVans, RandomAddon)
    end)
end, "admin")
...
RegisterServerEvent('qb-vehicleshop:server:buy:vehicle')
AddEventHandler('qb-vehicleshop:server:buy:vehicle', function(Vehicle, Price)
    local src = source
    ...
    if BankBalance >= Price then
        ...
        Framework.Functions.ExecuteSql(false, "INSERT INTO `characters_vehicles` (`citizenid`, `vehicle`, `plate`, `garage`, `state`, `mods`, `metadata`) VALUES ('"..Player.PlayerData.citizenid.."', '"..Vehicle.."', '"..Plate.."', '"..json.encode(GarageData).."', 'out', '{}', '"..json.encode(VehicleMeta).."')")
        TriggerClientEvent('qb-vehicleshop:client:spawn:bought:vehicle', src, Vehicle, Plate)
    else
        TriggerClientEvent('Framework:Notify', src, "You dont have enough cash in the bank..", 'error')
    end
end)
