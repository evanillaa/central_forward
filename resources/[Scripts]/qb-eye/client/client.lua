...
AddEventHandler('qb-eye:client:getin:trunk', function()
    ...
    if Vehicle ~= 0 and Distance < 5.0 then
            ...
            if not Config.TrunkData[Plate]['Busy'] then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
                    ...
                    if ClassData['CanEnter'] then
                        ...
                        Framework.Functions.Notify('Even hopped in a trunk of a motorcycle bruh?', 'error', 2500)
                    end
                else
                    Framework.Functions.Notify('Is the trunk even open?', 'error', 2500)
                end
            else
                Framework.Functions.Notify('There already is someone in the trunk.', 'error', 2500)
            end
        else
            if not Config.TrunkData[Plate]['Busy'] then
                if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
        ...
        Framework.Functions.Notify('No vehicle found', 'error', 2500)
    end
end)
...
AddEventHandler('qb-eye:client:getout:trunk', function()
    ...
    if GetVehicleDoorAngleRatio(Vehicle, 5) > 0 then
        ...
        Framework.Functions.Notify('Is the trunk open?', 'error', 2500)
    end
end)
