...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(5)
        DisableControlAction(0, Config.Keys["F2"], true)
        ...
        if LoggedIn then
           if IsDisabledControlJustPressed(0, Config.Keys["F2"]) and not Config.InventoryBusy then
               ...
               Framework.Functions.GetPlayerData(function(PlayerData)
                   if not PlayerData.metadata["isdead"] and not PlayerData.metadata["ishandcuffed"] then
                           ...
                           if vehicle ~= 0 and vehicle ~= nil then
                               ...
                               if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, trunkpos) < 1.0) and not IsPedInAnyVehicle(PlayerPedId()) then
                                   if GetVehicleDoorLockStatus(vehicle) < 2 then
                                       ...
                                       Framework.Functions.Notify("Vehicle is locked..", "error")
                                       return
                                   end
                               else
                                   CurrentVehicle = nil
                       ...
                       elseif DumpsterFound then
                           ...
                           TriggerServerEvent("qb-inventory:server:OpenInventory", "stash", Dumpster, {maxweight = 1000000, slots = 15})
                           TriggerEvent("qb-inventory:client:SetCurrentStash", Dumpster)
                           TriggerEvent('qb-inventory:client:open:anim')   
                       elseif JailContainerFound and exports['qb-prison']:GetInJailStatus() then
                           ...
                           TriggerServerEvent("qb-inventory:server:OpenInventory", "stash", Container, {maxweight = 1000000, slots = 15})
                           TriggerEvent("qb-inventory:client:SetCurrentStash", Container)
                           TriggerEvent('qb-inventory:client:open:anim')
           ...
           if IsControlJustPressed(0, Config.Keys["TAB"]) then
               ToggleHotbar(true)
           end
   
           if IsControlJustReleased(0, Config.Keys["TAB"]) then
               ToggleHotbar(false)
           end

           if IsDisabledControlReleased(0, Config.Keys["LEFTALT"]) and IsDisabledControlJustReleased(1, Config.Keys["1"]) then
               Framework.Functions.GetPlayerData(function(PlayerData)
                   if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] then
                       TriggerServerEvent("qb-inventory:server:UseItemSlot", 1)
                   end
               end)
           end
   
           if IsDisabledControlReleased(0, Config.Keys["LEFTALT"]) and IsDisabledControlJustReleased(1, Config.Keys["2"]) then
               Framework.Functions.GetPlayerData(function(PlayerData)
                   if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] then
                       TriggerServerEvent("qb-inventory:server:UseItemSlot", 2)
                   end
               end)
           end
   
           if IsDisabledControlReleased(0, Config.Keys["LEFTALT"]) and IsDisabledControlJustReleased(1, Config.Keys["3"]) then
               Framework.Functions.GetPlayerData(function(PlayerData)
                   if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] then
                       TriggerServerEvent("qb-inventory:server:UseItemSlot", 3)
                   end
               end)
           end
   
           if IsDisabledControlReleased(0, Config.Keys["LEFTALT"]) and IsDisabledControlJustReleased(1, Config.Keys["4"]) then
               Framework.Functions.GetPlayerData(function(PlayerData)
                   if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] then
                       TriggerServerEvent("qb-inventory:server:UseItemSlot", 4)
                   end
               end)
           end
   
           if IsDisabledControlReleased(0, Config.Keys["LEFTALT"]) and IsDisabledControlJustReleased(1, Config.Keys["5"]) then
               Framework.Functions.GetPlayerData(function(PlayerData)
                   if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] then
                       TriggerServerEvent("qb-inventory:server:UseItemSlot", 5)
                   end
               end)
           end
        end
...
RegisterNUICallback('RobMoney', function(data, cb)
    TriggerServerEvent("qb-police:server:rob:player", data.TargetId)
end)
...
RegisterNUICallback('UseItemShiftClick', function(slot)
    Framework.Functions.GetPlayerData(function(PlayerData)
        if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] then
            ...
            Citizen.Wait(250)
            TriggerServerEvent("qb-inventory:server:UseItemSlot", slot.slot)
        end
    end)
end)
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    ...
    Framework.Functions.TriggerCallback('qb-weapons:server:RemoveAttachment', function(NewAttachments)
...
RegisterNUICallback("CloseInventory", function(data, cb)
    ...
    TriggerServerEvent('qb-inventory:server:set:inventory:disabled', false)
    --Citizen.Wait(2600)
end)
...
RegisterNUICallback("combineItem", function(data)
 ...
 TriggerEvent('qb-inventory:client:ItemBox', Framework.Shared.Items[data.reward], 'add')
end)
...
RegisterNUICallback('combineWithAnim', function(data)
    ...
    Citizen.SetTimeout(1250, function()
        ...
        Framework.Functions.Progressbar("combine_anim", animText, animTimeout, false, true, {
            ...
            TriggerServerEvent('qb-inventory:server:combineItem', combineData.reward, data.requiredItem, data.usedItem, combineData.RemoveToItem)
        end, function() -- Cancel
            Config.InventoryBusy = false
            StopAnimTask(PlayerPedId(), aDict, aLib, 1.0)
...
-- // Events \\ --
RegisterNetEvent('qb-inventory:client:close:inventory')
AddEventHandler('qb-inventory:client:close:inventory', function()
    TriggerServerEvent('qb-inventory:server:set:inventory:disabled', false)
...
RegisterNetEvent('qb-inventory:client:set:busy')
AddEventHandler('qb-inventory:client:set:busy', function(bool)
    Config.InventoryBusy = bool
end)

RegisterNetEvent('qb-inventory:client:CheckOpenState')
AddEventHandler('qb-inventory:client:CheckOpenState', function(type, id, label)
...
RegisterNetEvent("qb-inventory:bag:UseBag")
AddEventHandler("qb-inventory:bag:UseBag", function()
    ...
    Framework.Functions.Progressbar("use_bag", "Opening Bag", 5000, false, true, {
        ...
        TriggerServerEvent("qb-inventory:server:OpenInventory", "stash", "bag_"..Framework.Functions.GetPlayerData().citizenid)
        TriggerEvent("qb-inventory:client:SetCurrentStash", "bag_"..Framework.Functions.GetPlayerData().citizenid)
        TriggerServerEvent("InteractSound_SV:PlayOnSource", "stash", 0.5)
        TaskPlayAnim(ped, "clothingshirt", "exit", 8.0, 1.0, -1, 49, 0, 0, 0, 0)
        Framework.Functions.Notify("Bag has been opened successfully", "success")
    end)
end)

RegisterNetEvent('qb-weapons:client:set:current:weapon')
AddEventHandler('qb-weapons:client:set:current:weapon', function(data, bool)
    if data ~= false then
...
RegisterNetEvent('qb-inventory:client:ItemBox')
AddEventHandler('qb-inventory:client:ItemBox', function(itemData, type)
    SendNUIMessage({
        action = "itemBox",
        item = itemData,
        type = type
...
RegisterNetEvent('qb-inventory:client:busy:status')
AddEventHandler('qb-inventory:client:busy:status', function(bool)
    CanOpenInventory = bool
end)

RegisterNetEvent('qb-inventory:client:requiredItems')
AddEventHandler('qb-inventory:client:requiredItems', function(items, bool)
    local itemTable = {}
    if bool then
...
RegisterNetEvent('qb-inventory:server:RobPlayer')
AddEventHandler('qb-inventory:server:RobPlayer', function(TargetId)
    SendNUIMessage({
        action = "RobMoney",
        TargetId = TargetId,
    })
...
AddEventHandler("qb-inventory:client:OpenInventory", function(inventory, other)
...
RegisterNetEvent("qb-inventory:client:UpdatePlayerInventory")
AddEventHandler("qb-inventory:client:UpdatePlayerInventory", function(isError)
...
RegisterNetEvent("qb-inventory:client:CraftItems")
AddEventHandler("qb-inventory:client:CraftItems", function(itemName, itemCosts, amount, toSlot, points)
    SendNUIMessage({
        action = "close",
    })
    Config.InventoryBusy = true
    Framework.Functions.Progressbar("repair_vehicle", "Crafting ..", (math.random(2000, 5000) * amount), false, true, {
        ...
        TriggerEvent('qb-inventory:client:ItemBox', Framework.Shared.Items[itemName], 'add')
        Config.InventoryBusy = false
	end, function() -- Cancel
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
        Framework.Functions.Notify("Mislukt!", "error")
...
RegisterNetEvent("qb-inventory:client:CraftWeapon")
AddEventHandler("qb-inventory:client:CraftWeapon", function(itemName, itemCosts, amount, toSlot, ItemType)
    SendNUIMessage({
        action = "close",
    })
    Config.InventoryBusy = true
    Framework.Functions.Progressbar("repair_vehicle", "Crafting ....", (math.random(10000, 12000) * amount), false, true, {
		...
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
        TriggerServerEvent("qb-inventory:server:CraftWeapon", itemName, itemCosts, amount, toSlot, ItemType)
        TriggerEvent('qb-inventory:client:ItemBox', Framework.Shared.Items[itemName], 'add')
        Config.InventoryBusy = false
	end, function() -- Cancel
		StopAnimTask(PlayerPedId(), "mini@repair", "fixing_a_player", 1.0)
...
RegisterNetEvent("qb-inventory:client:UseWeapon")
AddEventHandler("qb-inventory:client:UseWeapon", function(weaponData)
    ...
    elseif weaponName == "weapon_stickybomb" then
        ...
        TriggerEvent('qb-weapons:client:set:current:weapon', weaponData)
    ...
    elseif weaponName == "weapon_molotov" then
        ...
        TriggerEvent('qb-weapons:client:set:current:weapon', weaponData)
        Framework.Functions.TriggerCallback("qb-weapon:server:GetWeaponAmmo", function(result)
...
RegisterNetEvent("qb-inventory:client:CheckWeapon")
AddEventHandler("qb-inventory:client:CheckWeapon", function(weaponName)
    if currentWeapon == weaponName then 
        TriggerEvent('qb-assets:ResetHolster')
        SetCurrentPedWeapon(PlayerPedId(), GetHashKey("WEAPON_UNARMED"), true)
        RemoveAllPedWeapons(PlayerPedId(), true)
        currentWeapon = nil
    end
...
RegisterNetEvent("qb-inventory:client:AddDropItem")
AddEventHandler("qb-inventory:client:AddDropItem", function(dropId, player)
...
RegisterNetEvent("qb-inventory:client:RemoveDropItem")
AddEventHandler("qb-inventory:client:RemoveDropItem", function(dropId)
    Drops[dropId] = nil
end)

RegisterNetEvent("qb-inventory:client:DropItemAnim")
AddEventHandler("qb-inventory:client:DropItemAnim", function()
    SendNUIMessage({
        action = "close",
    })
...
RegisterNetEvent("qb-inventory:client:ShowId")
AddEventHandler("qb-inventory:client:ShowId", function(sourceId, citizenid, character)
...
RegisterNetEvent("qb-inventory:client:ShowDriverLicense")
AddEventHandler("qb-inventory:client:ShowDriverLicense", function(sourceId, citizenid, character)
    local sourcePos = GetEntityCoords(GetPlayerPed(GetPlayerFromServerId(sourceId)), false)
    local pos = GetEntityCoords(PlayerPedId(), false)
    if (GetDistanceBetweenCoords(pos.x, pos.y, pos.z, sourcePos.x, sourcePos.y, sourcePos.z, true) < 2.0) then
        TriggerEvent('chat:addMessage', {
...
RegisterNetEvent('qb-inventory:client:open:anim')
AddEventHandler('qb-inventory:client:open:anim', function()
  exports['qb-assets']:RequestAnimationDict('pickup_object')
