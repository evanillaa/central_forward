...
RegisterNetEvent('qb-items:client:use:joint')
AddEventHandler('qb-items:client:use:joint', function()
  Citizen.SetTimeout(1000, function()
    ...
    Framework.Functions.Progressbar("smoke-joint", "Joint opsteken..", 2500, false, true, {
     ...
     disableCombat = true,
        ...
        TriggerEvent("qb-inventory:client:ItemBox", Framework.Shared.Items["joint"], "remove")
        TriggerEvent('qb-items:client:joint:effect')
        if IsPedInAnyVehicle(PlayerPedId(), false) then
            TriggerEvent('animations:client:EmoteCommandStart', {"smoke3"})
        else
            TriggerEvent('animations:client:EmoteCommandStart', {"smokeweed"})
        end
        TriggerEvent("qb-police:client:SetStatus", "weedsmell", 200)
      end, function() -- Cancel
        TriggerEvent('qb-inventory:client:set:busy', false)
...
RegisterNetEvent("qb-items:client:use:coke")
AddEventHandler("qb-items:client:use:coke", function()
   Citizen.SetTimeout(1000, function()
     TriggerEvent('qb-inventory:client:set:busy', true)
     Framework.Functions.Progressbar("snort_coke", "Sleutel Puntje..", math.random(5000, 8000), false, true, {
         ...
         TriggerEvent("qb-police:client:SetStatus", "widepupils", 200)
         CokeBagEffect()
     end, function() -- Cancel
         TriggerEvent('qb-inventory:client:set:busy', false)
         StopAnimTask(PlayerPedId(), "anim@amb@nightclub@peds@", "missfbi3_party_snort_coke_b_male3", 1.0)
...
RegisterNetEvent("qb-items:client:use:lsd")
AddEventHandler("qb-items:client:use:lsd", function()
  Citizen.SetTimeout(1000, function()
    TriggerEvent('qb-inventory:client:set:busy', true)
...
RegisterNetEvent('qb-items:client:joint:effect')
AddEventHandler('qb-items:client:joint:effect', function()
  OnWeed = true
  while OnWeed do
    Citizen.Wait(1)
    if WeedTime > 0 then
      WeedTime = WeedTime - 1
      TriggerServerEvent('qb-hud:server:remove:stress', math.random(2, 6))
      Citizen.Wait(1000)
    end
    if WeedTime == 0 then
       OnWeed = false
