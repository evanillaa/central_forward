...
Framework.Functions.CreateCallback('qb-lapraces:server:GetRacingLeaderboards', function(source, cb)
    cb(Races)
end)
...
AddEventHandler('qb-lapraces:server:FinishPlayer', function(RaceData, TotalTime, TotalLaps, BestLap)
    ...
    if Races[RaceData.RaceId].Records ~= nil and next(Races[RaceData.RaceId].Records) ~= nil then
        if BLap < Races[RaceData.RaceId].Records.Time then
            ...
            TriggerClientEvent('qb-phone:client:RaceNotify', src, 'You have the WR van '..RaceData.RaceName..' disconnected with a time of: '..SecondsToClock(BLap)..'!')
        end
    else
        Races[RaceData.RaceId].Records = {
            Time = BLap,
        ...
        Framework.Functions.ExecuteSql(false, "UPDATE `lapraces` SET `records` = '"..json.encode(Races[RaceData.RaceId].Records).."' WHERE `raceid` = '"..RaceData.RaceId.."'")
        TriggerClientEvent('qb-phone:client:RaceNotify', src, 'You have the WR van '..RaceData.RaceName..' put down with a time of: '..SecondsToClock(BLap)..'!')
    end
...
AddEventHandler('qb-lapraces:server:CreateLapRace', function(RaceName)
    ...
    if IsWhitelisted(Player.PlayerData.citizenid) then
        if IsNameAvailable(RaceName) then
            ...
            TriggerClientEvent('Framework:Notify', source, 'There is already a race with this name.', 'error')
        end
    else
        TriggerClientEvent('Framework:Notify', source, 'You are not authorized to do this.', 'error')
    end
end)

Framework.Functions.CreateCallback('qb-lapraces:server:GetRaces', function(source, cb)
    cb(AvailableRaces)
end)

Framework.Functions.CreateCallback('qb-lapraces:server:GetListedRaces', function(source, cb)
    cb(Races)
end)

Framework.Functions.CreateCallback('qb-lapraces:server:GetRacingData', function(source, cb, RaceId)
    cb(Races[RaceId])
end)

Framework.Functions.CreateCallback('qb-lapraces:server:HasCreatedRace', function(source, cb)
    cb(HasOpenedRace(Framework.Functions.GetPlayer(source).PlayerData.citizenid))
end)

Framework.Functions.CreateCallback('qb-lapraces:server:IsAuthorizedToCreateRaces', function(source, cb, TrackName)
...
Framework.Functions.CreateCallback('qb-lapraces:server:GetTrackData', function(source, cb, RaceId)
    Framework.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `citizenid` = '"..Races[RaceId].Creator.."'", function(result)
...
AddEventHandler('qb-lapraces:server:JoinRace', function(RaceData)
    ...
    if CurrentRace ~= nil then
        ...
        if (AmountOfRacers - 1) == 0 then
            ...
            TriggerClientEvent('Framework:Notify', src, 'You were the only one left in the race. Race ended.', 'error')
    ...
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('qb-phone:client:RaceNotify', creatorsource, string.sub(Player.PlayerData.charinfo.firstname, 1, 1)..'. '..Player.PlayerData.charinfo.lastname..' joined the race!')
    end
end)
...
AddEventHandler('qb-lapraces:server:LeaveRace', function(RaceData)
    ...
    local creatorsource = Framework.Functions.GetPlayerByCitizenId(AvailableRaces[AvailableKey].SetupCitizenId).PlayerData.source
    if creatorsource ~= Player.PlayerData.source then
        TriggerClientEvent('qb-phone:client:RaceNotify', creatorsource, string.sub(Player.PlayerData.charinfo.firstname, 1, 1)..'. '..Player.PlayerData.charinfo.lastname..' has left the race!')
    end
    local AmountOfRacers = 0
    for k, v in pairs(Races[RaceData.RaceId].Racers) do
        AmountOfRacers = AmountOfRacers + 1
    ...
    if Races[RaceId] ~= nil then
        if not Races[RaceId].Waiting then
            if not Races[RaceId].Started then
                ...
                TriggerClientEvent('Framework:Notify', source, 'The race is already ongoing.', 'error')
            end
        else
            TriggerClientEvent('Framework:Notify', source, 'The race is already ongoing.', 'error')
        end
    else
        TriggerClientEvent('Framework:Notify', source, 'This race does not exist.', 'error')
    end
end)
...
Framework.Commands.Add("togglesetup", "Turn on / off Racing setup", {}, false, function(source, args)
    local Player = Framework.Functions.GetPlayer(source)

    if IsWhitelisted(Player.PlayerData.citizenid) then
        Config.RaceSetupAllowed = not Config.RaceSetupAllowed
        if not Config.RaceSetupAllowed then
            TriggerClientEvent('Framework:Notify', source, 'No more races can be created.', 'error')
        else
            TriggerClientEvent('Framework:Notify', source, 'Races can be created again.', 'success')
        end
    else
        TriggerClientEvent('Framework:Notify', source, 'You are not authorized to do this.', 'error')
    end
...
Framework.Functions.CreateCallback('qb-lapraces:server:CanRaceSetup', function(source, cb)
