...
RegisterNetEvent('qb-materials:client:search:trash')
AddEventHandler('qb-materials:client:search:trash', function()
    if CurrentBin ~= nil then
      if not Config.OpenedBins[CurrentBin] then
        Framework.Functions.Progressbar("search-trash", "Searching..", math.random(10000, 12500), false, true, {
            ...
            Framework.Functions.TriggerCallback('qb-materials:server:get:reward', function() end)
        ...
        Framework.Functions.Notify("You already searched this trashbin..", "error")
      end
    end
end)
