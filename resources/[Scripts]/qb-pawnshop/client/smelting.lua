                ...
                if not Config.Smelting then
                    if Config.CanTake then
                        ...
                        if IsControlJustReleased(0, 38) then
                            ...
                            Config.MeltTime = 300
                            TriggerServerEvent('qb-pawnshop:server:redeem:gold:bars')
                        end
                    else
                        ...
                        if IsControlJustReleased(0, 38) then
                            Framework.Functions.Progressbar("sell-gold", "Melting Gold..", math.random(10000, 15000), false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                            }, {}, {}, {}, function() -- Done
                                TriggerServerEvent('qb-pawnshop:server:smelt:gold')
                            end, function() -- Cancel
...
RegisterNetEvent('qb-pawnshop:client:start:process')
AddEventHandler('qb-pawnshop:client:start:process', function()
    if not Config.Smelting then
...
RegisterNetEvent('qb-pawnshop:server:reset:smelter')
AddEventHandler('qb-pawnshop:server:reset:smelter', function()
    Config.Smelting = false
    Config.CanTake = false
    Config.SmeltTime = 300 -- Adjust the time if you like 300 sec = 5 minutes. https://discord.gg/exaETQfMEy
end)