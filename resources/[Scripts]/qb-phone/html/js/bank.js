...
$(document).on('click', '.bank-app-account', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "Bank", "Bank account number copied!", "#badc58", 1750);
});
			...
			if (amount > 0) {
				$.post('http://qb-phone/TransferMoney', JSON.stringify({
					...
					if (data.CanTransfer) {
						...
						QB.Phone.Notifications.Add("fas fa-university", "Bank", "You have €"+amount+",- Transferred!", "#badc58", 1500);
					} else {
						QB.Phone.Notifications.Add("fas fa-university", "Bank", "You do not have enough funds!", "#badc58", 1500);
					}
				})
				QB.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
			} else {
				QB.Phone.Notifications.Add("fas fa-university", "Bank", "HQuantity must not be less than 0", "#badc58", 1500);
			 }
        } else {
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('http://qb-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                QB.Phone.Notifications.Add("fas fa-university", "Bank", "You have &euro;"+InvoiceData.amount+" paid!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-university", " Bank", "No IBAN is bound to this contact!", "#badc58", 2500);
    }
    QB.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});