...
function SetupPlayerHouses(Houses) {
    ...
    if (Houses.length > 0) {
        $.each(Houses, function(id, house){
            ...
            var HouseDetails = '<i class="fas fa-key"></i>&nbsp;&nbsp;' + TotalKeyholders + '&nbsp&nbsp&nbsp&nbsp';
            if (house.garage.length > 0) {
                HouseDetails = '<i class="fas fa-key"></i>&nbsp;&nbsp;' + TotalKeyholders + '&nbsp&nbsp&nbsp&nbsp';
            }
...
$(document).on('click', '#myhouse-option-transfer-confirm', function(e){
    ...
    $.post('http://qb-phone/TransferCid', JSON.stringify({
        ...
        if (CanTransfer) {
            ...
            QB.Phone.Notifications.Add("fas fa-home", "Huizen", "Unknown Citizen", "#27ae60", 2500);
