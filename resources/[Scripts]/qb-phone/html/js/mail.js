...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    if (Mails !== null && Mails !== undefined) {
        if (Mails.length > 0) {
            ...
            $(".mail-list").html('<p class="nomails">No new emails..</p>');
        }
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        QB.Phone.Notifications.Add("fas fa-ad", "Advertisement", "You cant place an empty AD!", "#ff8f1a", 2000);
    }
});
...
QB.Phone.Functions.RefreshAdverts = function(Adverts) {
    ...
    if (Adverts.length > 0 || Adverts.length == undefined) {
        ...
        $(".advert-list").html("");
        var element = '<div class="advert"><span class="advert-sender">No advertisements found</span></div>';
        $(".advert-list").append(element);
    }
}