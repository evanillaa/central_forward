...
QB.Phone.Functions.RefreshAutoscout = function(Cars) {
    ...
    if (Cars !== null) {
        $.each(Cars, function(i, car){
            CarsElement = '<div class="occasion-vehicle" id="car-'+car.plate+'"><div class="occasion-vehicle-name">'+car.vehicle+'</div><div class="occasion-vehicle-price">€'+car.salePrice+'</div><div class="occasion-vehicle-plate">Licenseplate: '+car.plate+'</div></div>'
...
$(document).on('click', '#buy-veh', function(e){
    ...
    if (forSaleVehicle !== "") {
        ...
        $.post('http://qb-phone/BuyVehicle', JSON.stringify({
            plate: forSaleVehicle,
            citizenid: forSaleCitizenid,
            price: forSalePrice,
        }));
        $.post('http://qb-phone/LoadAutoscout', JSON.stringify({}), function(Cars){
            QB.Phone.Functions.RefreshAutoscout(Cars);
        });
    } else {
        QB.Phone.Notifications.Add("fas fa-times-circle", "Oeps...", "Something went wrong try again...", "#e74c3c", 2000);
    }
});
