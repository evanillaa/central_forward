...
Framework.Functions.CreateCallback('qb-phone:server:GetPhoneData', function(source, cb)
    local src = source
    local Player = Framework.Functions.GetPlayer(src)
    if Player ~= nil then
        local PhoneData = {
                    ...
                    if cars ~= nil then
                        ...
                        for k,v in pairs(cars) do
                            cars = {
                                ...
                                vehicle = Framework.Shared.Vehicles[v.vehicle]['Name'], --exports['qb-garages']:GetVehicleName(v.vehicle)
                                plate = v.plate,
...
Framework.Functions.CreateCallback('qb-phone:server:GetCallState', function(source, cb, ContactData)
...
AddEventHandler('qb-phone:server:buy:chosen:vehicle', function(VehiclePlate, CitizenId, SellPrice)
    ...
    if Player.PlayerData.money['bank'] >= SellPrice then
        ...
        if TargetPlayer ~= nil then
           ...
           Framework.Functions.ExecuteSql(false, "UPDATE characters_vehicles SET `citizenid` = '" .. Player.PlayerData.citizenid .. "', `garage` = 'Rode Parking', `state` = 'in', `forSale` = '0', `salePrice` = '0' WHERE `plate` = '" ..VehiclePlate.. "'")
           TriggerClientEvent('qb-phone:client:send:email:bought:vehicle', src, VehiclePlate) 
           TriggerClientEvent('qb-phone:client:send:email:sold:vehicle', TargetPlayer.PlayerData.source, SellPrice, VehiclePlate) 
           TriggerClientEvent('Framework:Notify', src, "Vehicle with licenseplate '" .. VehiclePlate .. "' has been purchased for €'" .. SellPrice .. "'", "success")
        else
            Framework.Functions.ExecuteSql(false, 'SELECT `money` FROM characters_metadata WHERE citizenid ="'..CitizenId..'"',function(result)
                if result ~= nil then
                 ...
                 local MailData = {
                    sender = "Autoscout24",
                    subject = "Your advertention",
                    message = "Dear reader,<br/><br/>You recieve this email due to your vehicle with the following information:<br><br>License Plate: <strong>" ..VehiclePlate.. "</strong> <br>Sale price: <strong>€"..SellPrice.. '</strong><br><br>Your vehicle has been sold, the money has been wired to your bank account.<br><br>Kind regards,<br>Autoscout24',
                    button = {}
                 }
                 ...
                 TriggerClientEvent('qb-phone:client:send:email:bought:vehicle', src, VehiclePlate)
                 TriggerClientEvent('Framework:Notify', src, "Voertuig met Numberplaat '" .. VehiclePlate .. "' gekocht voor €'" .. SellPrice .. "'", "success")
                end
            end)
        end
    else
...
Framework.Functions.CreateCallback('qb-phone:server:load:autoscout', function(source, cb)
 Framework.Functions.ExecuteSql(false, "SELECT * FROM characters_vehicles WHERE forSale = '1'", function(cars)
     if cars ~= nil then
         local CarsData = {}
         for k,v in pairs(cars) do
            ...
            local VehicleData = Framework.Shared.Vehicles[v.vehicle]
             cars = {
                 citizenid = v.citizenid,
                 --vehicle = exports['qb-garages']:GetVehicleName(v.vehicle),
...
Framework.Functions.CreateCallback('qb-phone:server:PayInvoice', function(source, cb, sender, amount, invoiceId)
    local src = source
    local Ply = Framework.Functions.GetPlayer(src)
...
Framework.Functions.CreateCallback('qb-phone:server:DeclineInvoice', function(source, cb, sender, amount, invoiceId)
    local src = source
    local Ply = Framework.Functions.GetPlayer(src)
...
Framework.Functions.CreateCallback('qb-phone:server:GetContactPictures', function(source, cb, Chats)
...
Framework.Functions.CreateCallback('qb-phone:server:GetContactPicture', function(source, cb, Chat)
...
Framework.Functions.CreateCallback('qb-phone:server:GetPicture', function(source, cb, number)
...
Framework.Functions.CreateCallback('qb-phone:server:GetCurrentDrivers', function(source, cb)
    local drivers = {}
    for k, v in pairs(Framework.Functions.GetPlayers()) do
        local Player = Framework.Functions.GetPlayer(v)
        if Player ~= nil then
...
AddEventHandler('qb-phone:server:UpdateTweets', function(NewTweets, TweetData, Type)
    Tweets = NewTweets
    local TwtData = TweetData
    local src = source
    TriggerClientEvent('qb-phone:client:UpdateTweets', -1, src, Tweets, TwtData, Type)
end)
...
AddEventHandler('qb-phone:server:TransferMoney', function(iban, amount)
    ...
    Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_metadata` WHERE `charinfo` LIKE '%"..iban.."%'", function(result)
        if result[1] ~= nil then
            ...
            TriggerClientEvent('Framework:Notify', src, "Dit rekeningNumber bestaat niet!", "error")
        end
    end)
end)
...
Framework.Functions.CreateCallback('qb-phone:server:FetchResult', function(source, cb, search)
    local src = source
        ...
        if result[1] ~= nil then
            for k, v in pairs(result) do
                ...
                table.insert(searchData, {
                    ...
                    appartmentname = exports['qb-appartments']:GetAppartmentName(globals["appartment-data"].Name),
...
Framework.Functions.CreateCallback('qb-phone:server:GetVehicleSearchResults', function(source, cb, search)
    local src = source
    local search = escape_sqli(search)
    local searchData = {}
...
Framework.Functions.CreateCallback('qb-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData = {}
    if plate ~= nil then 
...
Framework.Functions.CreateCallback('qb-phone:server:GetGarageVehicles', function(source, cb)
    local Player = Framework.Functions.GetPlayer(source)
    local Vehicles = {}
    ...
    Framework.Functions.ExecuteSql(false, "SELECT * FROM `characters_vehicles` WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(result)
        if result[1] ~= nil then
            for k, v in pairs(result) do
                ...
                if v.vehicle ~= nil then
                    vehdata = {
                        --fullname = exports['qb-garages']:GetVehicleName(v.vehicle),
                       -- model = exports['qb-garages']:GetVehicleName(v.vehicle),
...
Framework.Functions.CreateCallback('qb-phone:server:GetInvoiceData', function(source, cb)
    local Player = Framework.Functions.GetPlayer(source)
    local InvoiceData = {}
    Framework.Functions.ExecuteSql(false, "SELECT * FROM characters_bills WHERE `citizenid` = '"..Player.PlayerData.citizenid.."'", function(invoices)
        if invoices[1] ~= nil then
...
RegisterServerEvent('qb-phone:server:add:invoice')
AddEventHandler('qb-phone:server:add:invoice', function(TargetPlayer, Amount, Sender, Type)
    local PhoneData = {}
...
Framework.Functions.CreateCallback('qb-phone:server:HasPhone', function(source, cb)
    local Player = Framework.Functions.GetPlayer(source)
    if Player ~= nil then
...
Framework.Functions.CreateCallback('qb-phone:server:GetCurrentLawyers', function(source, cb)
    local Lawyers = {}
    for k, v in pairs(Framework.Functions.GetPlayers()) do
        local Player = Framework.Functions.GetPlayer(v)
        if Player ~= nil then
            if Player.PlayerData.job.name == "lawyer" or Player.PlayerData.job.name == "realestate" or Player.PlayerData.job.name == "mechanic" and Player.PlayerData.job.onduty then
