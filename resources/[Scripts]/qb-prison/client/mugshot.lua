...
RegisterNetEvent('qb-prison:client:set:in:jail')
AddEventHandler('qb-prison:client:set:in:jail', function(Name, Months, CitizenId, Date)
...
function JailIntro(Name, Months, CitizenId, Date)
    ...
    TriggerEvent('qb-sound:client:play', 'handcuff', 0.4)
    ...
    TriggerEvent('qb-sound:client:play', 'jail-photo', 0.4)
    Citizen.Wait(3000) 
    TriggerEvent('qb-sound:client:play', 'jail-photo', 0.4)
    Citizen.Wait(3000) 
    ...
    TriggerServerEvent("qb-prison:server:set:jail:items", Months)
    TriggerEvent('qb-prison:client:enter:prison', Months, true)
    Framework.Functions.Notify("You have been sentenced to jail for "..Months.." months..", "error", 6500)
end
...
function AddMugshot(Name, Months, CitizenId, Date)
 ...
 exports['qb-assets']:RequestAnimationDict("mp_character_creation@lineup@male_a")
