...
Framework.Commands.Add("jail", "Send a citizen to jail", {{name="id", help="Player ID"}, {name="tijd", help="Time to serve in numbers!"}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "judge" then
        if JailPlayer ~= nil then
         ...
         if Time > 0 then
            ...
            TriggerClientEvent('qb-prison:client:set:in:jail', JailPlayer.PlayerData.source, Name, Time, JailPlayer.PlayerData.citizenid, os.date('%d-'..'%m-'..'%y'))
         end
      end
    end
end)
...
RegisterServerEvent('qb-prison:server:set:jail:state')
AddEventHandler('qb-prison:server:set:jail:state', function(Time)
 local Player = Framework.Functions.GetPlayer(source)
...
RegisterServerEvent('qb-prison:server:set:jail:leave')
AddEventHandler('qb-prison:server:set:jail:leave', function()
  local Player = Framework.Functions.GetPlayer(source)
...
RegisterServerEvent('qb-prison:server:set:jail:items')
AddEventHandler('qb-prison:server:set:jail:items', function(Time)
    local src = source
...
RegisterServerEvent('qb-prison:server:get:items:back')
AddEventHandler('qb-prison:server:get:items:back', function()
    local src = source
    local Player = Framework.Functions.GetPlayer(src)
    Citizen.Wait(100)
...
RegisterServerEvent('qb-prison:server:find:reward')
AddEventHandler('qb-prison:server:find:reward', function(reward)
   local Player = Framework.Functions.GetPlayer(source)
   Player.Functions.AddItem(reward, 1)
   TriggerClientEvent("qb-inventory:client:ItemBox", source, Framework.Shared.Items[reward], "add")
end)

RegisterServerEvent('qb-prison:server:set:alarm')
AddEventHandler('qb-prison:server:set:alarm', function(bool)
    TriggerClientEvent('qb-prison:client:set:alarm', -1, bool)
end)