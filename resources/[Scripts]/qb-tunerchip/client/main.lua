...
RegisterNUICallback('save', function(data)
    Framework.Functions.TriggerCallback('qb-tunerchip:server:HasChip', function(HasChip)
        if HasChip then
            local ped = PlayerPedId()
            local veh = GetVehiclePedIsUsing(ped)
            setVehData(veh, data)
            Framework.Functions.Notify('Tuning saved!', 'error')
...
AddEventHandler('qb-tunerchip:server:TuneStatus', function()
    ...
    Framework.Functions.TriggerCallback('qb-tunerchip:server:GetStatus', function(status)
        if status then
...
RegisterNUICallback('reset', function(data)
    ...
    local veh = GetVehiclePedIsUsing(ped)
    resetVeh(veh)
    Framework.Functions.Notify('Tunerchip: Reset!', 'error')
end)
...
AddEventHandler('qb-tunerchip:client:openChip', function()
    ...
    if inVehicle then
        Framework.Functions.Progressbar("connect_laptop", "Connecting Tuner Laptop..", 2000, false, true, {
        ...
        Framework.Functions.Notify("You are not sitting in a vehicle..", "error")
    end
end)
