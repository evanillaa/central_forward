...
AddEventHandler('Framework:Client:OnPlayerLoaded', function()
  Citizen.SetTimeout(1250, function()
      ...
      Citizen.Wait(100)
      Framework.Functions.TriggerCallback("qb-vehiclekeys:server:get:key:config", function(config)
          Config = config
      end)
      isLoggedIn = true
  end)
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsPedInAnyVehicle(GetPlayerPed(-1), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1), true), -1) == GetPlayerPed(-1) then
            ...
            Framework.Functions.TriggerCallback("qb-vehiclekeys:server:has:keys", function(HasKey)
                if HasKey then
                    HasCurrentKey = true
                ...
                if GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= nil and GetVehiclePedIsTryingToEnter(GetPlayerPed(-1)) ~= 0 then
                    ...
                    if Driver ~= 0 and not IsPedAPlayer(Driver) then
                       if IsEntityDead(Driver) then
                           IsRobbing = true
                           Framework.Functions.Progressbar("rob_keys", "Retrieving keys..", 3000, false, true,
                            {}, {}, {}, {}, function()
...
RegisterNetEvent('qb-vehiclekeys:client:toggle:engine')
AddEventHandler('qb-vehiclekeys:client:toggle:engine', function()
 ...
 Framework.Functions.TriggerCallback("qb-vehiclekeys:server:has:keys", function(HasKey)
     if HasKey then
         ...
         Framework.Functions.Notify("You dont have the keys of this vehicle..", 'error')
     end
 end, Plate)
end)

RegisterNetEvent('qb-vehiclekeys:client:set:keys')
AddEventHandler('qb-vehiclekeys:client:set:keys', function(Plate, CitizenId, bool)
    Config.VehicleKeys[Plate] = {['CitizenId'] = CitizenId, ['HasKey'] = bool}
    LastVehicle = nil
end)

RegisterNetEvent('qb-vehiclekeys:client:give:key')
AddEventHandler('qb-vehiclekeys:client:give:key', function(TargetPlayer)
    ...
    Framework.Functions.TriggerCallback("qb-vehiclekeys:server:has:keys", function(HasKey)
        if HasKey then
            if Player ~= -1 and Player ~= 0 and Distance < 2.3 then
                 Framework.Functions.Notify("You gave your vehicle keys of: "..Plate, 'success')
                 TriggerServerEvent('qb-vehiclekeys:server:give:keys', GetPlayerServerId(Player), Plate, true)
            else
                Framework.Functions.Notify("No citizen nearby", 'error')
            end
        else
            Framework.Functions.Notify("You dont have keys of this vehicle..", 'error')
        end
    end, Plate)
end)
   ...
   Framework.Functions.TriggerCallback("qb-vehiclekeys:server:has:keys", function(HasKey)
      if not HasKey then
       if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
          exports['qb-assets']:RequestAnimationDict("anim@amb@clubhouse@tutorial@bkr_tut_ig3@")
          ...
          exports['qb-lockpick']:OpenLockpickGame(function(Success)
             if Success then
                 ...
                 TriggerEvent('qb-vehicleley:client:blink:lights', Vehicle)
                 TriggerServerEvent("qb-sound:server:play:distance", 5, "car-unlock", 0.2)
                 StopAnimTask(GetPlayerPed(-1), "anim@amb@clubhouse@tutorial@bkr_tut_ig3@", "machinic_loop_mechandplayer", 1.0)
             else
                if IsAdvanced then
...
function SetVehicleKey(Plate, bool)
 TriggerServerEvent('qb-vehiclekeys:server:set:keys', Plate, bool)
end
...
function ToggleLocks()
 ...
 if Vehicle ~= nil and Vehicle ~= 0 then
    ...
    if VehDistance <= 2.2 then
        Framework.Functions.TriggerCallback("qb-vehiclekeys:server:has:keys", function(HasKey)
         if HasKey then
            exports['qb-assets']:RequestAnimationDict("anim@mp_player_intmenu@key_fob@")
            ...
            if VehicleLocks == 1 then
                ...
                TriggerServerEvent("qb-sound:server:play:distance", 5, "car-lock", 0.2)
            else
                Citizen.Wait(450)
                SetVehicleDoorsLocked(Vehicle, 1)
                ClearPedTasks(GetPlayerPed(-1))
...
RegisterNetEvent('qb-vehicleley:client:blink:lights')
AddEventHandler('qb-vehicleley:client:blink:lights', function(Vehicle)
