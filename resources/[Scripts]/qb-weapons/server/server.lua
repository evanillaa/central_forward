...
Framework.Commands.Add("addammo", "Give bullets for the weapon you are holding", {{name="1", help="1"}}, true, function(source, args)
    if args[1] ~= nil then
      TriggerClientEvent('qb-weapons:client:set:ammo', source, args[1])
    end
end, "admin")
...
Framework.Functions.CreateUseableItem("pistol-ammo", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('qb-weapons:client:reload:ammo', source, 'AMMO_PISTOL', 'pistol-ammo')
    end
end)
...
Framework.Functions.CreateUseableItem("rifle-ammo", function(source, item)
	local Player = Framework.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('qb-weapons:client:reload:ammo', source, 'AMMO_RIFLE', 'rifle-ammo')
    end
end)
...
Framework.Functions.CreateUseableItem("smg-ammo", function(source, item)
	local Player = Framework.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('qb-weapons:client:reload:ammo', source, 'AMMO_SMG', 'smg-ammo')
    end
end)
...
Framework.Functions.CreateUseableItem("shotgun-ammo", function(source, item)
	local Player = Framework.Functions.GetPlayer(source)
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('qb-weapons:client:reload:ammo', source, 'AMMO_SHOTGUN', 'shotgun-ammo')
    end
end)

Framework.Functions.CreateUseableItem("pistol_suppressor", function(source, item)
    local Player = Framework.Functions.GetPlayer(source)
    TriggerClientEvent("qb-weapons:client:EquipAttachment", source, item, "suppressor")
end)

Framework.Functions.CreateUseableItem("pistol_extendedclip", function(source, item)
    local Player = Framework.Functions.GetPlayer(source)
    TriggerClientEvent("qb-weapons:client:EquipAttachment", source, item, "extendedclip")
end)
...
Framework.Functions.CreateUseableItem("rifle_flashlight", function(source, item)
    local Player = Framework.Functions.GetPlayer(source)
    TriggerClientEvent("qb-weapons:client:EquipAttachment", source, item, "flashlight")
end)

Framework.Functions.CreateUseableItem("rifle_grip", function(source, item)
    local Player = Framework.Functions.GetPlayer(source)
    TriggerClientEvent("qb-weapons:client:EquipAttachment", source, item, "grip")
end)

Framework.Functions.CreateUseableItem("rifle_scope", function(source, item)
    local Player = Framework.Functions.GetPlayer(source)
    TriggerClientEvent("qb-weapons:client:EquipAttachment", source, item, "scope")
end)

RegisterServerEvent('qb-weapons:server:UpdateWeaponQuality')
AddEventHandler('qb-weapons:server:UpdateWeaponQuality', function(data, RepeatAmount)
    local src = source
            ...
            if WeaponSlot.info.quality ~= nil then
                for i = 1, RepeatAmount, 1 do
                    if WeaponSlot.info.quality - DecreaseAmount > 0 then
                        ...
                        TriggerClientEvent('qb-inventory:client:UseWeapon', src, data)
                        TriggerClientEvent('Framework:Notify', src, "Your weapon is broken.", "error")
                        break
                    end
                end
            else
...
RegisterServerEvent("qb-weapons:server:EquipAttachment")
AddEventHandler("qb-weapons:server:EquipAttachment", function(ItemData, CurrentWeaponData, AttachmentData)
    local src = source
    local Player = Framework.Functions.GetPlayer(src)
    ...
    if Inventory[CurrentWeaponData.slot] ~= nil then
        if Inventory[CurrentWeaponData.slot].info.attachments ~= nil and next(Inventory[CurrentWeaponData.slot].info.attachments) ~= nil then
            ...
            if not HasAttach then
                ...
                TriggerClientEvent("qb-weapons:client:addAttachment", src, AttachmentData.component)
                Player.Functions.SetInventory(Player.PlayerData.items)
                ...
                TriggerClientEvent("Framework:Notify", src, "You already have one "..AttachmentData.label:lower().."  on your gun..", "error", 3500)
            end
        else
...
RegisterServerEvent("qb-weapons:server:SetWeaponQuality")
AddEventHandler("qb-weapons:server:SetWeaponQuality", function(data, hp)
    local src = source
    local Player = Framework.Functions.GetPlayer(src)
...
RegisterServerEvent("qb-weapons:server:UpdateWeaponAmmo")
AddEventHandler('qb-weapons:server:UpdateWeaponAmmo', function(CurrentWeaponData, amount)
    local src = source
    local Player = Framework.Functions.GetPlayer(src)
...
Framework.Functions.CreateCallback("qb-weapon:server:GetWeaponAmmo", function(source, cb, WeaponData)
    local Player = Framework.Functions.GetPlayer(source)
    local retval = 0
...
Framework.Functions.CreateCallback('qb-weapons:server:RemoveAttachment', function(source, cb, AttachmentData, ItemData)
    local src = source
    local Player = Framework.Functions.GetPlayer(src)
    local Inventory = Player.PlayerData.items
    ...
    if Inventory[ItemData.slot] ~= nil then
        if Inventory[ItemData.slot].info.attachments ~= nil and next(Inventory[ItemData.slot].info.attachments) ~= nil then
            ...
            if HasAttach then
                ...
                TriggerClientEvent('qb-inventory:client:ItemBox', src, Framework.Shared.Items[AttachmentComponent.item], "add")
