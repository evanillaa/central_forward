...
Citizen.CreateThread(function()
	...
	TriggerServerEvent("qb-boombox:joined")
end)
...
RegisterNUICallback("loadServer",function(data, cb)
	...
	local tempTable = {
		...
		TriggerServerEvent("qb-boombox:loadSpeaker", tempTable[1])
end)


RegisterNetEvent("qb-boombox:loadSpeakerClient")
AddEventHandler("qb-boombox:loadSpeakerClient", function(speaker)
	cacheSpeaker[speaker.speaker] = speaker
end)


RegisterNetEvent("qb-boombox:joined")
AddEventHandler("qb-boombox:joined", function(speaker)
	cacheSpeaker = speaker
end)

RegisterNetEvent("qb-boombox:removeClient")
AddEventHandler("qb-boombox:removeClient", function(speaker)
...
RegisterNetEvent("qb-boombox:switchVideoClient")
AddEventHandler("qb-boombox:switchVideoClient", function(id, videoStatus, time)
	cacheSpeaker[id].switch = true
	cacheSpeaker[id].videoStatus = videoStatus
	cacheSpeaker[id].time = time - cacheSpeaker[id].time
end)

RegisterNetEvent("qb-boombox:changeVolClient")
AddEventHandler("qb-boombox:changeVolClient", function(id, vol)
                ...
                if v.id ~= nil then
					...
					if dist < 250.0 then
						...
						elseif playing == k and onSwitch then
							...
							TriggerServerEvent("qb-boombox:switchVideo", k, videoStatus, newTime)
						...
						elseif playing == k and onVolChange then
							...
							TriggerServerEvent("qb-boombox:changeVol", k, volchange)
			...
			for k, v in pairs(cacheSpeaker) do
				...
				if #(v.coords - GetEntityCoords(PlayerPedId())) < 2.0 then
					...
					if IsControlJustPressed(0, 47) then
						...
						TriggerServerEvent("qb-boombox:removeSpeaker", currentId)
...
RegisterNetEvent("qb-boombox:place")
AddEventHandler("qb-boombox:place", function()
	local plyrcoords, forward = GetEntityCoords(PlayerPedId()), GetEntityForwardVector(PlayerPedId())
	local spawncoords = (plyrcoords + forward * 1.0)
	tooClose = false
	if #cacheSpeaker > 0 then
		...
		if not tooClose then
			...
			TriggerServerEvent("qb-boombox:placedSpeaker", spawncoords, speaker)
