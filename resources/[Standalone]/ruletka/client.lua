...
AddEventHandler('esx_roulette:start_game', function(action, amount)
	...
	if game_during == false then
		...
		Framework.Functions.Notify("You have "..amount .." euro on "..kolorBetu ..". put.", "success")
		game_during = true
		...
		Framework.Functions.Notify("You have won "..win .." euros!", "success")
				TriggerServerEvent('esx_roulette:givemoney', action, amount)
			else
				Framework.Functions.Notify("Verloren", "error")
				--ESX.ShowNotification('Not this time. Try again! Good luck!')
			end
		elseif action == 'red' then
			local win = amount * 2
			if has_value(red, randomNumber) then
				Framework.Functions.Notify("You have won "..win .." euro!", "success")
				TriggerServerEvent('esx_roulette:givemoney', action, amount)
			else
				Framework.Functions.Notify("You lost!", "success")
				--ESX.ShowNotification('Not this time. Try again! Good luck!')
			end
